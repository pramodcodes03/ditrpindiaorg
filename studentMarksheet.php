...
$inst_id = isset($_REQUEST['inst_id']) ? $_REQUEST['inst_id'] : '';
...
if ($user_id != '' && !empty($user_id)) {
	...
	include_once('admin/include/classes/exam.class.php');
	$exam 	= new  exam();
	include('admin/include/classes/exammultisub.class.php');
	$exammultisub = new exammultisub();
	include('admin/include/classes/coursetypingexam.class.php');
	...
	if ($resB != '') {
		...
		while ($dataB = $resB->fetch_assoc()) {
			extract($dataB);
			// print_r($dataB); exit();
			if ($course != '0' || $course_multi_sub != '0') {
				$imageId = $dataB['inst_id'];
				$marksheet_image = $dataB['marksheet_image'];
				$marksheet_image      = BACKGROUND_IMAGE_PATH . '/' . $imageId . '/' . $marksheet_image;
			}
			if ($course_typing != '' && $course_typing != '0') {
				$imageId = $dataB['inst_id'];
				$typingmarksheet_image = $dataB['typingmarksheet_image'];
				$marksheet_image      = BACKGROUND_IMAGE_PATH . '/' . $imageId . '/' . $typingmarksheet_image;
			}
		}
	}
	...
	//$id = base64_decode($id);
	if ($course !== '' && !empty($course) && $course != '0') {
		$cond = " AND COURSE_ID='$course' AND STUDENT_ID='$user_id' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
	}
	if ($course_multi_sub !== '' && !empty($course_multi_sub) && $course_multi_sub != '0') {
		$cond = " AND MULTI_SUB_COURSE_ID='$course_multi_sub' AND STUDENT_ID='$user_id' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
	}
	if ($course_typing !== '' && !empty($course_typing) && $course_typing != '0') {
		$cond = " AND TYPING_COURSE_ID='$course_typing' AND STUDENT_ID='$user_id' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
	}
	...
	if ($TYPING_COURSE_ID != '' && !empty($TYPING_COURSE_ID) && $TYPING_COURSE_ID != 0) {
		...
		if ($res2 != '') {
			...
			while ($data2 = $res2->fetch_assoc()) {
				...
				$total_marks_of_all_subjects = $EXAM_TOTAL_MARKS * $srno1;

...
class coursetypingexam extends access
{
	...
	*/
	public function add_exam()
	{
		...
		$minimum_marks 		= parent::test(isset($_POST['minimum_marks']) ? $_POST['minimum_marks'] : '');
		...
		if ($totalmarks == '') $errors['totalmarks'] = 'Maximum marks is required!';
		if ($minimum_marks == '') $errors['minimum_marks'] = 'Minimum marks is required!';
		...
		if (! empty($errors)) {
			...
			$tableName 	= "course_typing_exam_structure";
			$tabFields 	= "(EXAM_ID, TYPING_COURSE_ID, COURSE_SUBJECT_ID, TOTAL_MARKS,MINIMUM_MARKS,EXAM_MODE_TYPE,SHOW_RESULT,DEMO_TEST, ACTIVE,CREATED_BY, CREATED_ON)";
			$insertVals	= "(NULL, '$courseid', '$subjectid', '$totalmarks','$minimum_marks','$exam_mode','$showresult', '$demotest', '$status','$created_by',NOW())";
	...
	*/
	public function update_exam($exam_id)
	{
		...
		if (!empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "course_typing_exam_structure";
			$setValues 	= "TYPING_COURSE_ID ='$courseid', COURSE_SUBJECT_ID='$subjectid', TOTAL_MARKS='$totalmarks',MINIMUM_MARKS='$minimum_marks',EXAM_MODE_TYPE='$exam_mode', SHOW_RESULT='$showresult',DEMO_TEST='$demotest', ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
	...
	{
		$data = '';
		$sql = "SELECT A.*,  (SELECT TYPING_COURSE_NAME  FROM courses_typing WHERE TYPING_COURSE_ID=A.TYPING_COURSE_ID) AS COURSE_NAME_MODIFY, (SELECT TYPING_COURSE_CODE FROM courses_typing WHERE TYPING_COURSE_ID=A.TYPING_COURSE_ID) AS TYPING_COURSE_CODE , (SELECT CONCAT(TYPING_COURSE_SUBJECT_NAME,' ',TYPING_COURSE_SPEED) FROM courses_typing_subjects WHERE TYPING_COURSE_SUBJECT_ID=A.COURSE_SUBJECT_ID) AS SUBJECT_NAME_MODIFY FROM  course_typing_exam_structure A WHERE A.DELETE_FLAG=0 ";
	...
	public function add_quebank_multi_sub()
	{
		...
		if (! empty($errors)) {
			...
			if ($quebankfile != '') {
				...
				$tabFields 	= "(QUEBANK_ID, MULTI_SUB_COURSE_ID, COURSE_SUBJECT_ID, FILE_NAME,IMG_ZIP_FILE, ACTIVE,CREATED_BY, CREATED_ON)";
				$insertVals	= "(NULL, '$courseid', '$subjectid', '$file_name','$img_zip_file_name','$status','$created_by',NOW())";
				$insertSql	= parent::insertData($tableName, $tabFields, $insertVals);
				$exSql = parent::execQuery($insertSql);
				if ($exSql) {
					...
					@mkdir(QUEBANK_PATH_FOR_MULTI_SUB . '/' . $last_insert_id, 0777, true);
					...
					if (file_exists($courseImgPathFile)) {
						...
						while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
							...
							if ($i > 0) {
								...
								$tabFields1 	= "(QUESTION_ID,QUEBANK_ID,MULTI_SUB_COURSE_ID,COURSE_SUBJECT_ID, QUESTION,IMAGE, OPTION_A, OPTION_B, OPTION_C,OPTION_D,CORRECT_ANS, CREATED_BY, CREATED_ON)";
								$insertVals1	= "(NULL,'$last_insert_id','$courseid','$subjectid','$QUESTION','$IMAGE','$OPTION_A','$OPTION_B', '$OPTION_C', '$OPTION_D','$CORRECT_ANS', '$created_by',NOW())";
...
*/
	...
	public function add_question_multi_sub($quebank_id)
	{
		...
		if (! empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "multi_sub_exam_question_bank";
			$tabFields = "(QUEBANK_ID,COURSE_ID,COURSE_SUBJECT_ID,QUESTION,OPTION_A,OPTION_B,OPTION_C,OPTION_D,CORRECT_ANS,ACTIVE,CREATED_BY,CREATED_ON,CREATED_BY_IP";
	...
	public function list_student_exam_results_typing_list($id = '', $stud_id = '', $inst_id = '', $stud_course_id = '', $cond = '')
	{
		$data = '';
		$sql = "SELECT A.*,get_subject_title_typing(A.STUDENT_SUBJECT_ID) as SUBJECT_NAME,get_subject_speed_typing(A.STUDENT_SUBJECT_ID) as TYPING_COURSE_SPEED,B.TOTAL_MARKS FROM course_typing_exam_result A RIGHT JOIN course_typing_exam_structure B ON A.STUDENT_SUBJECT_ID = B.COURSE_SUBJECT_ID WHERE A.DELETE_FLAG=0 AND B.DELETE_FLAG=0 ";
		if ($id != '') {
			$sql .= " AND A.EXAM_RESULT_FINAL_ID='$id' ";
		}
		if ($inst_id != '') {
...
*/
	...
	public function add_practical_exam_marks_typing()
	{
		$errors 			= array();  // array to hold validation errors
		$data 				= array();
		$data['success'] 	= false;		// array to pass back data
		...
		if (! empty($errors)) {
			...
			if ($totalSubjectCount != '') {
				for ($i = 1; $i <= $totalSubjectCount; $i++) {
					...
					$typing_studcoursedetailid = parent::test(isset($_POST['typing_studcoursedetailid' . $i]) ? $_POST['typing_studcoursedetailid' . $i] : '');

					$typing_studentid 		= parent::test(isset($_POST['typing_studentid' . $i]) ? $_POST['typing_studentid' . $i] : '');

					$typing_instituteid 		= parent::test(isset($_POST['typing_instituteid' . $i]) ? $_POST['typing_instituteid' . $i] : '');

					$typing_examid 			= parent::test(isset($_POST['typing_examid' . $i]) ? $_POST['typing_examid' . $i] : '');

					$typing_coursesubid 			= parent::test(isset($_POST['typing_coursesubid' . $i]) ? $_POST['typing_coursesubid' . $i] : '');

					$typing_instcourseid 	= parent::test(isset($_POST['typing_instcourseid' . $i]) ? $_POST['typing_instcourseid' . $i] : '');

					$typing_examtitle 		= parent::test(isset($_POST['typing_examtitle' . $i]) ? $_POST['typing_examtitle' . $i] : '');

					$typing_examattempt 		= parent::test(isset($_POST['typing_examattempt' . $i]) ? $_POST['typing_examattempt' . $i] : '');

					$typing_totalmarks 	= parent::test(isset($_POST['typing_totalmarks' . $i]) ? $_POST['typing_totalmarks' . $i] : '');
					$typing_minimummarks 	= parent::test(isset($_POST['typing_minimummarks' . $i]) ? $_POST['typing_minimummarks' . $i] : '');

					$typing_examtype 			= parent::test(isset($_POST['typing_examtype' . $i]) ? $_POST['typing_examtype' . $i] : '');
					$typing_examstatus		= parent::test(isset($_POST['typing_examstatus' . $i]) ? $_POST['typing_examstatus' . $i] : '');

					$totaltheory    = parent::test(isset($_POST['totaltheory' . $i]) ? $_POST['totaltheory' . $i] : '');
					$minimum_marks    = parent::test(isset($_POST['minimum_marks' . $i]) ? $_POST['minimum_marks' . $i] : '');
					$thobt 			= parent::test(isset($_POST['thobt' . $i]) ? $_POST['thobt' . $i] : '');
					$created_by = $_SESSION['user_fullname'];
					$created_on_ip = $_SESSION['ip_address'];

					$tableName 	= "course_typing_exam_result";

					$tabFields 	= "(EXAM_RESULT_ID, STUD_COURSE_ID,STUDENT_ID,STUDENT_SUBJECT_ID,INSTITUTE_ID,EXAM_ID, 
						INSTITUTE_COURSE_ID, EXAM_TITLE,EXAM_TOTAL_MARKS,MARKS_OBTAINED, EXAM_TYPE, CREATED_BY, CREATED_ON, CREATED_ON_IP,MINIMUM_MARKS)";

					$insertVals	= "(NULL,'$typing_studcoursedetailid','$typing_studentid','$typing_coursesubid','$typing_instituteid','$typing_examid','$typing_instcourseid','$typing_examtitle','$typing_totalmarks','$thobt','$typing_examtype','$created_by',NOW(),'$created_on_ip','$minimum_marks')";
				...
				if ($exSql) {
					//$last_id = parent::last_id();					
					$sqlUpd 	= "UPDATE student_course_details SET EXAM_STATUS='3' WHERE STUD_COURSE_DETAIL_ID='$typing_studcoursedetailid'";
					parent::execQuery($sqlUpd);
					...
					$tableName1 	= "course_typing_exam_result_final";
					...
					$insertVals1	= "(NULL,'$typing_studcoursedetailid','$typing_studentid','$typing_instituteid','$typing_instcourseid','$typing_examtitle','$total_marks','$total_obt_marks ','$percentage','$grade_multi','$result_status_multi','$created_by',NOW(),'$created_on_ip')";
	...
	public function update_practical_exam_marks_typing()
	{
		$errors 			= array();  // array to hold validation errors
		$data 				= array();
		...
		if (! empty($errors)) {
			...
			if ($totalSubjectCount != '') {
				for ($i = 1; $i <= $totalSubjectCount; $i++) {
					...
					MINIMUM_MARKS='$minimum_marks',UPDATED_BY='$created_by', UPDATED_ON=NOW(),UPDATED_ON_IP='$created_on_ip'";

					$whereClause 	= " WHERE EXAM_RESULT_ID='$typing_examid' ";
					$exc 			= parent::updateData($tableName, $setValues, $whereClause);
					$res			= parent::execQuery($exc);
				}
				if ($res) {
					...
					$typing_examresultfinalid 		= parent::test(isset($_POST['typing_examresultfinalid']) ? $_POST['typing_examresultfinalid'] : '');
					$total_obt_marks 		= parent::test(isset($_POST['total_obt_marks']) ? $_POST['total_obt_marks'] : '');
					$total_marks 		= parent::test(isset($_POST['total_marks']) ? $_POST['total_marks'] : '');
					$percentage 		= parent::test(isset($_POST['percentage']) ? $_POST['percentage'] : '');
					$grade_multi 		= parent::test(isset($_POST['grade_multi']) ? $_POST['grade_multi'] : '');
					...
					$whereClause1 	= " WHERE EXAM_RESULT_FINAL_ID='$typing_examresultfinalid' ";
	...
	{
		$data = '';
		$sql = "SELECT EXAM_RESULT_TYPING_ID FROM certificate_requests WHERE DELETE_FLAG=0 ";
		if ($cert_req_id != '') {

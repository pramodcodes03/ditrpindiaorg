...
class course extends access
{
	...
	*/
	public function add_course()
	{
		...
		$coursesubject 		= parent::test(isset($_POST['coursesubject']) ? $_POST['coursesubject'] : '');
		...
		if ($coursesubject == '')
			$errors['coursesubject'] = 'Course Subjects is required!';
		...
		if (! empty($errors)) {
			...
			$tableName 	= "courses";
			$tabFields 	= "(COURSE_ID, COURSE_CODE,COURSE_AWARD, COURSE_DURATION, COURSE_NAME,COURSE_DETAILS,COURSE_ELIGIBILITY,ACTIVE,CREATED_BY, CREATED_ON,COURSE_SUBJECTS,VIDEO1,VIDEO2)";
			$insertVals	= "(NULL, UPPER('S-$coursecode'),'$award', UPPER('$duration'), UPPER('$coursename'),'$detail','$eligibility','$status','$created_by',NOW(),'$coursesubject','$video1','$video2')";
			...
			if ($exSql) {
				...
				if ($filecount1 >= 1) {
					for ($j = 0; $j < $filecount1; $j++) {
						...
						if ($plan != '' && $fees != '') {
							...
							$tabFields4 	= "(COURSE_PLAN_FEES_ID,COURSE_ID,PLAN_ID,COURSE_FEES,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
				...
				$tabFields2 	= "(INSTITUTE_COURSE_ID, INSTITUTE_ID, COURSE_ID, MULTI_SUB_COURSE_ID,COURSE_FEES,COURSE_SUBJECTS,ACTIVE, CREATED_BY,CREATED_ON)";
				$insertVals2	= "(NULL, '$inst_id', '$last_insert_id','0','$coursefees','$coursesubject','$status','$created_by',NOW())";
				$insertSql2		= parent::insertData($tableName2, $tabFields2, $insertVals2);
				$exSql2			= parent::execQuery($insertSql2);

				$courseImgPathDir 	= COURSE_MATERIAL_PATH . '/' . $last_insert_id . '/';

				//upload course image
				if ($course_img != '') {
					$ext 			= pathinfo($_FILES["course_img"]["name"], PATHINFO_EXTENSION);
					$file_name 		= $coursecode . '_logo.' . $ext;
					$setValues 		= "COURSE_IMAGE='$file_name'";
					$whereClause	= " WHERE COURSE_ID='$last_insert_id'";
				...
				if ($filecount >= 1) {
					for ($i = 0; $i < $filecount; $i++) {
						...
						if ($coursematerial != '') {
							...
							$tabFields3 	= "(FILE_ID,COURSE_ID,FILE_NAME,FILE_MIME,FILE_LABEL,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
	...
	*/
	public function update_course($course_id)
	{
		...
		if (!empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "courses";
			$setValues 	= "COURSE_CODE=UPPER('$coursecode'), COURSE_AWARD=UPPER('$award'), COURSE_DURATION=UPPER('$duration'), COURSE_NAME=UPPER('$coursename'),COURSE_DETAILS='$detail',COURSE_ELIGIBILITY='$eligibility',ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW(),COURSE_SUBJECTS='$coursesubject',VIDEO1='$video1',VIDEO2='$video2'";
			...
			$setValues1 	= "COURSE_FEES='$coursefees',COURSE_SUBJECTS='$coursesubject',ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
			$whereClause1	= " WHERE COURSE_ID='$course_id'";
			$updateSql1		= parent::updateData($tableName1, $setValues1, $whereClause1);
			$exSql1			= parent::execQuery($updateSql1);

			$courseImgPathDir = COURSE_MATERIAL_PATH . '/' . $course_id . '/';
			...
			if ($filecount1 >= 1) {
				for ($j = 0; $j < $filecount1; $j++) {
					...
					if ($course_plan_fees_id == '') {
						...
						$setValues4 	= "COURSE_ID='$course_id', PLAN_ID='$plan', COURSE_FEES='$fees',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
			...
			if ($filecount >= 1) {
				for ($i = 0; $i < $filecount; $i++) {
					...
					if ($coursematerial != '') {
						...
						$ext 			= pathinfo($_FILES["coursematerial" . $i]["name"], PATHINFO_EXTENSION);
						$file_name 		= $coursecode . '_' . mt_rand(0, 123456789) . '.' . $ext;
						$tabFields3 	= "(FILE_ID,COURSE_ID,FILE_NAME,FILE_MIME,FILE_LABEL,ACTIVE,CREATED_BY,CREATED_ON)";
	...
	{
		$data = '';
		$sql = "SELECT A.*, get_course_title_modify(A.COURSE_ID) as COURSE_NAME_MODIFY,(SELECT B.AWARD FROM course_awards B WHERE B.AWARD_ID=A.COURSE_AWARD) AS COURSE_AWARD_NAME FROM courses A WHERE A.DELETE_FLAG=0 ";
	...
	//student all courses not purchase
	public function list_courses_not_purchase($course_id = '', $condition = '')
	{
		$data = '';
		$sql = "SELECT A.* FROM institute_courses A WHERE A.DELETE_FLAG=0 AND A.ACTIVE=1 ";
	...
	public function get_course_docs_all($course_id = '', $display = true)
	{
		$img = '';
		$data = array();
		$target = '';

		$sql = "SELECT FILE_ID,FILE_NAME,FILE_LABEL,COURSE_ID FROM courses_files WHERE 1";
		if ($course_id != '')
			$sql .= " AND COURSE_ID='$course_id'";
		$sql .= ' ORDER BY FILE_ID DESC';
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$img .= '<table id="order-listing" class="table"><thead><tr> 
			<th>File Name</th>
			<th>Action</th>
		    </tr></thead>';
			...
			while ($rec = $res->fetch_assoc()) {
				...
				if ($FILE_NAME != '') {
					if (!$display) {
						$data1 = array("file_id" => $FILE_ID, "file_name" => $FILE_NAME, "file_label" => $FILE_LABEL, "course_id" => $COURSE_ID);
						array_push($data, $data1);
					} else {

						$fileLink = COURSE_MATERIAL_PATH . '/' . $COURSE_ID . '/' . $FILE_NAME;

						$img .= '<tr id="file-area' . $FILE_ID . '">
									<td><a href="' . $fileLink . '" target="_blank">' . $FILE_LABEL . '</a></td>
									<td>
										<a href="javascript:void(0)" title= "Delete File" onclick="deleteCourseFile(' . $FILE_ID . ',' . $COURSE_ID . ')" class="btn btn-danger table-btn"><i class="mdi mdi-delete"></i></a>
										&nbsp;&nbsp;
	...
	//video files
	public function get_course_videos_all($course_id = '', $display = true)
	{
		$img = '';
		$data = array();
		$target = '';

		$sql = "SELECT id,course_id,video_link,title FROM course_videos WHERE 1";
		if ($course_id != '')
			$sql .= " AND course_id='$course_id'";
		$sql .= ' ORDER BY id DESC';
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$img .= '<table id="order-listing1" class="table"><thead><tr> 
			<th>Name</th>
			<th>Video Link</th>
			<th>Action</th>
		</tr></thead>';
			while ($rec = $res->fetch_assoc()) {
				...
				if ($id != '') {
					if (!$display) {
						$data1 = array("id" => $id, "title" => $title, "video_link" => $video_link, "course_id" => $course_id);
						array_push($data, $data1);
					} else {
						$img .= '
							<tr id="videos' . $id . '">
									...
									<td>' . $video_link . '</td>
									<td>
										<a href="javascript:void(0)" title= "Delete Video Link" onclick="deleteCourseVideos(' . $id . ',' . $COURSE_ID . ')" class="btn btn-primary btn-sm delete-icon"><i class="fa fa-trash-o"></i>Delete</a>
									</td>
								  </tr>';
					}
				}
	...
	public function get_course_plans($course_id = '', $display = true)
	{
		$img = '';
		$data = array();
		$target = '';

		$sql = "SELECT A.*, B.PLAN_NAME	 FROM course_plan_fees A inner join institute_plans B ON A.PLAN_ID=B.PLAN_ID   WHERE 1";
		if ($course_id != '')
			$sql .= " AND A.COURSE_ID='$course_id'";
			...
			while ($rec = $res->fetch_assoc()) {
				...
				if ($PLAN_ID != '') {
					...
					$img .= '<tr id="file-area' . $COURSE_PLAN_FEES_ID . '">
								...
								<td>' . $COURSE_FEES . '</td>
								<td>
									<a href="javascript:void(0)" title= "Delete File" onclick="deleteCoursePlan(' . $COURSE_PLAN_FEES_ID . ',' . $COURSE_ID . ')" class="delete-icon"><i class="fa fa-trash-o"></i></a>
								</td>
							  </tr>';
				}
				array_push($data, $rec);
	...
	{
		$code = 'S-' . $code;
		$sql = "SELECT COURSE_CODE FROM courses WHERE COURSE_CODE='$code' AND DELETE_FLAG=0";
		if ($course_id != '')
			$sql .= " AND COURSE_ID!='$course_id'";
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
	...
	/* delete course file */
	public function delete_course_file($file_id, $course_id = '')
	{
		$sql = "DELETE FROM courses_files WHERE FILE_ID='$file_id'";
		if ($course_id != '')
			$sql .= " AND COURSE_ID='$course_id'";
		$res = parent::execQuery($sql);
	...
	public function delete_course_video($file_id, $course_id = '')
	{
		$sql = "DELETE FROM course_videos WHERE id ='$file_id'";
		if ($course_id != '')
			$sql .= " AND course_id='$course_id'";
		$res = parent::execQuery($sql);
		if ($res && parent::rows_affected() > 0) {
	...
	{
		$sql = "UPDATE courses SET ACTIVE='$flag',UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW() WHERE COURSE_ID='$course_id'";
		$res = parent::execQuery($sql);

		if ($res) {
			$sql1 = "UPDATE institute_courses SET ACTIVE='$flag',UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW() WHERE COURSE_ID='$course_id'";
	...
	public function list_added_courses($inst_id)
	{
		$data = '';
		$sql = "SELECT A.*,A.COURSE_FEES as EXAM_FEES,get_course_award_name(A.COURSE_AWARD) AS COURSE_AWARD_NAME,get_course_title_modify (A.COURSE_ID) COURSE_NAME_MODIFY, B.INSTITUTE_COURSE_ID, B.ACTIVE AS STATUS, B.COURSE_FEES AS INSTITUTE_COURSE_FEES,B.PLAN_ID, B.PLAN_FEES , B.MINIMUM_FEES AS MINIMUM_FEES FROM  courses A LEFT JOIN institute_courses B ON A.COURSE_ID=B.COURSE_ID  WHERE B.DELETE_FLAG=0  AND B.COURSE_TYPE=1 AND B.INSTITUTE_ID='$inst_id' ";
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$data = $res;
		}
	...
	public function list_added_courses_single($inst_course_id)
	{
		$data = '';
		$sql = "SELECT A.*,A.COURSE_FEES as EXAM_FEES,get_course_award_name(A.COURSE_AWARD) AS COURSE_AWARD_NAME, B.INSTITUTE_COURSE_ID, B.ACTIVE AS STATUS, B.COURSE_FEES AS INSTITUTE_COURSE_FEES, B.PLAN_FEES, B.MINIMUM_FEES AS MINIMUM_FEES FROM  courses A LEFT JOIN institute_courses B ON A.COURSE_ID=B.COURSE_ID  WHERE B.DELETE_FLAG=0  AND B.COURSE_TYPE=1 AND B.INSTITUTE_COURSE_ID='$inst_course_id'";
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$data = $res;
		}
	...
	public function list_notadded_courses($inst_id)
	{
		$data = '';
		$search = '';
		...
		$sql = "SELECT A.COURSE_ID FROM institute_courses A WHERE A.INSTITUTE_ID='$inst_id' AND A.DELETE_FLAG=0";
		$res = parent::execQuery($sql);
		...
		$where = '';
		if ($search != '') {
			$where = " AND A.COURSE_ID NOT IN($search) ";
		}
		$sql2 = "SELECT A.*, get_course_title_modify(A.COURSE_ID) AS COURSE_NAME_MODIFY,B.COURSE_FEES as PLAN_FEES,B.PLAN_ID FROM  courses A LEFT JOIN course_plan_fees B ON A.COURSE_ID = B.COURSE_ID WHERE  B.PLAN_ID='$plan_id' AND A.DELETE_FLAG=0 AND A.ACTIVE=1 $where";
	...
	{
		$sql = "SELECT * FROM institute_courses WHERE COURSE_ID='$course_id' AND INSTITUTE_ID='$inst_id' AND COURSE_TYPE='$course_type' AND DELETE_FLAG=0";
	...
	public function institute_add_aicpe_course()
	{
		//print_r($_POST);
		$res = '';
		$errors = array();  // array to hold validation errors
		...
		if (!empty($errors)) {
			...
			foreach ($course as $value) {
				...
				$courseminimumfees 	= isset($_POST['courseminimumfees_' . $value]) ? $_POST['courseminimumfees_' . $value] : 0;

				$sql = "SELECT institute_details.PLAN_ID, course_plan_fees.COURSE_FEES AS PLAN_FEES FROM institute_details inner join course_plan_fees on institute_details.PLAN_ID=course_plan_fees.PLAN_ID WHERE institute_details.INSTITUTE_ID=$institute_id AND course_plan_fees.COURSE_ID= $value LIMIT 0,1";
				$res = parent::execQuery($sql);
				if ($res && $res->num_rows > 0) {
					...
					if (!$this->check_course_in_institute($value, $institute_id, $course_type)) {
						$tableName = "institute_courses";
						$tabFields = "(INSTITUTE_COURSE_ID,INSTITUTE_ID,COURSE_ID,COURSE_TYPE,PLAN_ID,PLAN_FEES,EXAM_FEES,COURSE_FEES,CREATED_BY,CREATED_ON,CREATED_ON_IP,MINIMUM_FEES)";
						$insertValues = "(NULL, '$institute_id','$value','$course_type','$PLAN_ID','$PLAN_FEES','$PLAN_FEES','$coursefees','" . $_SESSION['user_name'] . "', NOW(), '" . $_SESSION['ip_address'] . "','$courseminimumfees')";
	...
	public function delete_institute_course($inst_course_id)
	{
		$res = "";
		...
		if ($res && $res->num_rows > 0) {
			...
			$COURSE_ID = $data['COURSE_ID'];
			if ($course_type == 2) {
				$updsql = "UPDATE non_courses SET DELETE_FLAG=1, ACTIVE=0, UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW(), UPDATED_ON_IP='" . $_SESSION['ip_address'] . "' WHERE COURSE_ID='$COURSE_ID'";
	...
	public function bulk_delete_inst_course($instCourseArr, $inst_id)
	{
	...
	public function get_course_multi_sub($course_id = '', $display = true)
	{
		...
		$sql = "SELECT A.* FROM multi_sub_courses_subjects A WHERE 1";
		if ($course_id != '')
	...
	public function add_award()
	{
		...
		$award_name 		= parent::test(isset($_POST['award_name']) ? $_POST['award_name'] : '');
		$status 			= parent::test(isset($_POST['status']) ? $_POST['status'] : '');
		...
		/* check validations */
		if ($award_name == '') $errors['award_name'] = 'Award name is required!';
		...
		if (! empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "course_awards";
			$tabFields 	= "(AWARD, ACTIVE,DELETE_FLAG, CREATED_BY,CREATED_ON)";
			$insertVals	= "('$award_name','1','0','$created_by',NOW())";
			$insertSql	= parent::insertData($tableName, $tabFields, $insertVals);
			$exSql		= parent::execQuery($insertSql);
			if ($exSql) {
				...
				parent::commit();
				$data['success'] = true;
				$data['message'] = 'Success! New Award name has been added successfully!';
			} else {
				parent::rollback();
	...
	public function update_award($id = '')
	{
		...
		if (!empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "course_awards";
			$setValues 	= " AWARD='$award_name',UPDATED_BY='$updated_by',UPDATED_ON=NOW()";
			$whereClause = " WHERE AWARD_ID ='$id'";
			$updateSql	= parent::updateData($tableName, $setValues, $whereClause);
			$exSql		= parent::execQuery($updateSql);

			parent::commit();
			$data['success'] = true;
			$data['message'] = 'Success! Award name has been updated successfully!';
		}
		return json_encode($data);
	}

	public function list_award($id = '', $condition = '')
	{
		$data = '';
		$sql = "SELECT A.* FROM course_awards A WHERE A.DELETE_FLAG=0 ";

		if ($id != '') {
			$sql .= " AND A.AWARD_ID ='$id' ";
		}
		if ($condition != '') {
			$sql .= " $condition ";
		}
	...
	{
		$sql = "UPDATE course_awards SET DELETE_FLAG=1, UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW() WHERE AWARD_ID='$id'";
		$res = parent::execQuery($sql);
		if ($res && parent::rows_affected() > 0) {
			return false;
		}

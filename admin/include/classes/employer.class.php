...
class employer extends access
{
	...
	*/
	public function add_employer()
	{
		...
		if ($creditcount != '') {
			if ($creditcount <= 0) {
				$errors['creditcount'] = 'Please enter valid credit amount.';
			}
		}
		if ($democount == '')
		...
		if ($democount != '') {
			if ($democount <= 0) {
				$errors['democount'] = 'Please enter valid demo amount.';
			}
		}
		if ($expirationdate == '')
		...
		if (!parent::valid_employer_email($email, ''))
			$errors['email'] = 'Sorry! Email is already used.';
		if (!$this->validate_employer_code($empcode, ''))
		...
		if (! empty($errors)) {
			...
			$tabFields 	= "(EMPLOYER_ID, EMPLOYER_CODE, EMPLOYER_COMPANY_NAME,EMPLOYER_NAME,DESIGNATION,ADDRESS_LINE1,ADDRESS_LINE2,MOBILE,EMAIL,CITY,STATE,COUNTRY,POSTCODE,DETAIL_DESCRIPTION,CREDIT,DEMO_PER,ACTIVE,VERIFIED, CREATED_BY, CREATED_ON,CREATED_ON_IP)";
			$insertVals	= "(NULL, '$empcode', UPPER('$empcmpname'), UPPER('$empname'),'$designation','$address1','$address2','$mobile','$email','$city','$state','$country','$postcode','$empdetails','$creditcount','$democount','$status','$verify','$created_by',NOW(), '" . $_SESSION['ip_address'] . "')";
			...
			if ($exSql) {
				...
				$insertVals2	= "(NULL, '$last_insert_id', '$email', MD5('$confpword'),'$role','$registrationdate','$expirationdate','$status','$created_by',NOW(), '" . $_SESSION['ip_address'] . "')";
				$insertSql2		= parent::insertData($tableName2, $tabFields2, $insertVals2);
				$exSql2			= parent::execQuery($insertSql2);

				if ($exSql2) {
					//$courseImgPathDir 		= 	EMPLOYER_PHOTO_PATH.'/'.$last_insert_id.'/';

					$bucket_directory = 'employer/' . $last_insert_id . '/';
					...
					if ($emplogo != '') {
						...
						$insertVals3	= "(NULL, '$last_insert_id', '$file_name','" . INST_LOGO . "','1',0,'$created_by',NOW(), '" . $_SESSION['ip_address'] . "')";
						$insertSql3		= parent::insertData($tableName3, $tabFields3, $insertVals3);
						$exec3   		= parent::execQuery($insertSql3);

						$s3_obj = new S3Class();
						$activityContent = $_FILES['emplogo']['name'];
						$fileTempName = $_FILES['emplogo']['tmp_name'];
						$new_width = 800;
						$new_height = 750;
						...
						$response = $s3_obj->putObjectFile($newFielName, BUCKET_NAME, $bucket_directory . '' . $file_name, S3::ACL_PUBLIC_READ, array(), array("Content-Type" => $_FILES["emplogo"]["type"]));
					...
					if ($emppassphoto != '') {
						...
						$tabFields3 	= "(FILE_ID,EMPLOYER_ID,FILE_NAME,FILE_LABEL,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON, CREATED_ON_IP)";
						$insertVals3	= "(NULL, '$last_insert_id', '$file_name','" . INST_OWNER_PHOTO . "','1',0,'$created_by',NOW(), '" . $_SESSION['ip_address'] . "')";
						$insertSql3		= parent::insertData($tableName3, $tabFields3, $insertVals3);
						$exec3   		= parent::execQuery($insertSql3);

						$s3_obj = new S3Class();
						$activityContent = $_FILES['emppassphoto']['name'];
						$fileTempName = $_FILES['emppassphoto']['tmp_name'];
						$new_width = 800;
						$new_height = 750;
						$image_p = imagecreatetruecolor($new_width, $new_height);
						$image = imagecreatefromstring(file_get_contents($fileTempName));
						...
						$response = $s3_obj->putObjectFile($newFielName, BUCKET_NAME, $bucket_directory . '' . $file_name, S3::ACL_PUBLIC_READ, array(), array("Content-Type" => $_FILES["emppassphoto"]["type"]));
					...
					if ($empphotoidproof != '') {
						...
						$tabFields3 	= "(FILE_ID,EMPLOYER_ID,FILE_NAME,FILE_LABEL,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON,CREATED_ON_IP)";
						$insertVals3	= "(NULL, '$last_insert_id', '$file_name','" . INST_PHOTO_PROOF . "','1',0,'$created_by',NOW(), '" . $_SESSION['ip_address'] . "')";
						$insertSql3		= parent::insertData($tableName3, $tabFields3, $insertVals3);
						$exec3   		= parent::execQuery($insertSql3);

						$s3_obj = new S3Class();
						$activityContent = $_FILES['empphotoidproof']['name'];
						$fileTempName = $_FILES['empphotoidproof']['tmp_name'];
						$new_width = 800;
						$new_height = 750;
						$image_p = imagecreatetruecolor($new_width, $new_height);
						$image = imagecreatefromstring(file_get_contents($fileTempName));
						...
						$response = $s3_obj->putObjectFile($newFielName, BUCKET_NAME, $bucket_directory . '' . $file_name, S3::ACL_PUBLIC_READ, array(), array("Content-Type" => $_FILES["empphotoidproof"]["type"]));
					...
					parent::commit();
					$data['success'] = true;
					$data['message'] = 'Success! New employer has been added successfully!';
				} else {
					parent::rollback();
	...
	*/
	public function update_employer($emp_id)
	{
		...
		if (! empty($errors)) {
			...
			$bucket_directory = 'employer/' . $employer_id . '/';
	...
	public function list_employer($employer_id = '', $condition = '')
	{
		$data = '';

...
class coursetyping extends access
{
	...
	@return: json
	*/
	public function add_course_typing()
	{
...
// 		$errors['minimumfees'] = 'Minimum fees is required!';
		 ...
		 if($course_img!='')
		  ...
		  if($filecount2>=1)
		  {
			  for($i=0; $i<$filecount2; $i++)
			  {
				...
				$speed 			= parent::test(isset($_POST['speed'.$i])?$_POST['speed'.$i]:'');
				
			  	if ($subject=='')
					$errors['subject'.$i] = 'Please Enter Subject!';  
				if ($speed=='')
					$errors['speed'.$i] = 'Please Enter Speed!';                                       
		  ...
		  if (! empty($errors)) {
			  ...
			  $data['message']  = 'Please correct all the errors.';
					...
					$tabFields 	= "(TYPING_COURSE_ID, TYPING_COURSE_CODE,TYPING_COURSE_DURATION, TYPING_COURSE_NAME,TYPING_COURSE_DETAILS,TYPING_COURSE_ELIGIBILITY,ACTIVE,CREATED_BY, CREATED_ON)";
					$insertVals	= "(NULL, UPPER('T-$coursecode'),UPPER('$duration'), UPPER('$coursename'),'$detail','$eligibility','$status','$created_by',NOW())";
					...
					if($exSql)
					{
						...
						$tabFields2 	= "(INSTITUTE_COURSE_ID, INSTITUTE_ID, COURSE_ID, MULTI_SUB_COURSE_ID,TYPING_COURSE_ID,COURSE_FEES,ACTIVE, CREATED_BY,CREATED_ON,MINIMUM_FEES)";
						$insertVals2	= "(NULL, '$inst_id', '0','0','$last_insert_id','$coursefees','$status','$created_by',NOW(),'$minimumfees')";
						$insertSql2		= parent::insertData($tableName2,$tabFields2,$insertVals2);
						$exSql2			= parent::execQuery($insertSql2);

						$courseImgPathDir 	= COURSE_WITH_TYPING_MATERIAL_PATH.'/'.$last_insert_id.'/';
						$tableName5 		= "courses_typing_subjects";
						if($filecount2>=1)
						{
							for($k=0; $k<$filecount2; $k++)
							{
							$subject 		= parent::test(isset($_POST['subject'.$k])?$_POST['subject'.$k]:'');
							$speed 			= parent::test(isset($_POST['speed'.$k])?$_POST['speed'.$k]:'');
						
							if($subject!='' && $speed!='')
							{
															
								$tabFields5 	= "(TYPING_COURSE_SUBJECT_ID,TYPING_COURSE_ID,TYPING_COURSE_SUBJECT_NAME,TYPING_COURSE_SPEED,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
								$insertVals5	= "(NULL, '$last_insert_id', '$subject','$speed','1','0','$created_by',NOW())";
								$insertSql5		= parent::insertData($tableName5,$tabFields5,$insertVals5);
								$exec5   		= parent::execQuery ($insertSql5);																
							}								
							}
						}
						$tableName4 		= "course_typing_plan_fees";
						if($filecount1>=1)
						{
							for($j=0; $j<$filecount1; $j++)
							{
							  ...
							  if($plan!='' && $fees!='')
							  {
								  ...
								  $tabFields4 	= "(COURSE_PLAN_FEES_ID,TYPING_COURSE_ID,PLAN_ID,COURSE_FEES,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
						...
						{								
							...
							$setValues 		= "TYPING_COURSE_IMAGE='$file_name'";
							$whereClause	= " WHERE TYPING_COURSE_ID ='$last_insert_id'";
						...
						parent::commit();
						$data['success'] = true;
						$data['message'] = 'Success! New typing course has been added successfully!';
					}else{
						parent::rollback();
	...
	@return: json
	*/
	public function update_course_typing($course_id)
	{
		  $errors = array();  // array to hold validation errors
		  $data = array();        // array to pass back data 
...
// 			$errors['minimumfees'] = 'Minimum fees is required!';
		...
		if($course_img!='')
		  ...
		  if (!empty($errors)) {
			  ...
			  $data['message']  = 'Please correct all the errors.';
					...
					parent::start_transaction();										
					$tableName 	= "courses_typing";
					$setValues 	= "TYPING_COURSE_CODE =UPPER('$coursecode'), TYPING_COURSE_DURATION=UPPER('$duration'), TYPING_COURSE_NAME =UPPER('$coursename'),TYPING_COURSE_DETAILS='$detail',TYPING_COURSE_ELIGIBILITY='$eligibility',ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
					$whereClause= " WHERE TYPING_COURSE_ID ='$course_id'";
					$updateSql	= parent::updateData($tableName,$setValues,$whereClause);				
					$exSql		= parent::execQuery($updateSql);

					$tableName1 	= "institute_courses";
					$setValues1 	= "COURSE_FEES='$coursefees',ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
					$whereClause1	= " WHERE TYPING_COURSE_ID='$course_id'";
					$updateSql1		= parent::updateData($tableName1,$setValues1,$whereClause1);			
					$exSql1			= parent::execQuery($updateSql1);

					$courseImgPathDir = COURSE_WITH_TYPING_MATERIAL_PATH.'/'.$course_id.'/';
					$tableName5 		= "courses_typing_subjects";
					if($filecount2>=1)
					  {
						  for($k=0; $k<$filecount2; $k++)
						  {
							$course_typing_id 			= parent::test(isset($_POST['course_typing_id'.$k])?$_POST['course_typing_id'.$k]:'');
							$subject 			= parent::test(isset($_POST['subject'.$k])?$_POST['subject'.$k]:'');
							$speed 			= parent::test(isset($_POST['speed'.$k])?$_POST['speed'.$k]:'');
							...
							if($course_typing_id=='')
							{
								...
								$tabFields5 	= "(TYPING_COURSE_SUBJECT_ID,TYPING_COURSE_ID,TYPING_COURSE_SUBJECT_NAME,TYPING_COURSE_SPEED,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
								$insertVals5	= "(NULL, '$course_id', '$subject','$speed','1','0','$created_by',NOW())";
								$insertSql5		= parent::insertData($tableName5,$tabFields5,$insertVals5); 
								$exec5   		= parent::execQuery ($insertSql5);																
							}else{
								$setValues5 	= "TYPING_COURSE_ID='$course_id', TYPING_COURSE_SUBJECT_NAME='$subject',TYPING_COURSE_SPEED='$speed',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
								$whereClause5= " WHERE TYPING_COURSE_SUBJECT_ID='$course_typing_id'";
					...
					if($filecount1>=1)
					  {
						  for($j=0; $j<$filecount1; $j++)
						  {
							...
							if($course_plan_fees_id=='')
							{
								...
								$setValues4 	= "TYPING_COURSE_ID='$course_id', PLAN_ID='$plan', COURSE_FEES='$fees',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
	...
	public function list_courses_typing($course_id='',$condition='')
	{
		$data = '';
		$sql= "SELECT A.* FROM courses_typing A WHERE A.DELETE_FLAG=0 ";
		
		if($course_id!='')
		{
			$sql .= " AND A.TYPING_COURSE_ID='$course_id' ";
		}
		if($condition!='')
		{
			$sql .= " $condition ";
	...
	public function get_course_plans_typing($course_id='', $display=true)
	{
		$img = '';
		$data= array();
		$target='';
		
		$sql ="SELECT A.*, B.PLAN_NAME	 FROM course_typing_plan_fees A inner join institute_plans B ON A.PLAN_ID=B.PLAN_ID   WHERE 1";
		if($course_id!='')
			$sql .= " AND A.TYPING_COURSE_ID='$course_id'";		
		...
		if($res && $res->num_rows>0)
		{
			...
			while($rec = $res->fetch_assoc())
			{
				...
				$TYPING_COURSE_ID 		= $rec['TYPING_COURSE_ID'];
				...
				if($PLAN_ID!='')
				{
					...
					$img .= '<tr id="file-area'.$COURSE_PLAN_FEES_ID.'">
								...
								<td>'.$COURSE_FEES.'</td>
								<td>
									<a href="javascript:void(0)" title= "Delete File" onclick="deleteCoursePlanTyping('.$COURSE_PLAN_FEES_ID.','.$TYPING_COURSE_ID.')" class="delete-icon"><i class="fa fa-trash-o"></i></a>
								</td>
							  </tr>';						
				}
				array_push($data, $rec);					
	...
	public function get_course_typing_subjects($course_id='', $display=true)
	{		
		$img = '';
		$data= array();
		$target='';	
		
		$sql ="SELECT A.* FROM courses_typing_subjects A WHERE ACTIVE = '1' AND DELETE_FLAG = '0'";
		if($course_id!='')
			$sql .= " AND A.TYPING_COURSE_ID='$course_id'";		
		$sql .= ' ORDER BY A.TYPING_COURSE_SUBJECT_ID ASC';
		//echo $sql;
		$res = parent::execQuery($sql);
		if($res && $res->num_rows>0)
		{
			...
			while($rec = $res->fetch_assoc())
			{
				$TYPING_COURSE_SUBJECT_ID  		= $rec['TYPING_COURSE_SUBJECT_ID '];
				$TYPING_COURSE_ID 				= $rec['TYPING_COURSE_ID'];
				$TYPING_COURSE_SUBJECT_NAME 	= $rec['TYPING_COURSE_SUBJECT_NAME'];	
				$TYPING_COURSE_SPEED 			= $rec['TYPING_COURSE_SPEED'];				
				if($TYPING_COURSE_SUBJECT_ID!='')
				{
										
					$img .= '<tr id="file-area'.$TYPING_COURSE_SUBJECT_ID.'">
								<td>'.$TYPING_COURSE_SUBJECT_NAME.'</td>	
								<td>'.$TYPING_COURSE_SPEED.'</td>							
								<td>
									<a href="javascript:void(0)" title= "Delete File" onclick="deleteCoursePlanTyping('.$TYPING_COURSE_SUBJECT_ID.','.$TYPING_COURSE_ID.')" class="delete-icon"><i class="fa fa-trash-o"></i></a>
								</td>
							  </tr>';						
				}
				array_push($data, $rec);					
	...
	{
		$sql = "SELECT TYPING_COURSE_CODE FROM courses_typing WHERE TYPING_COURSE_CODE ='$code' AND DELETE_FLAG=0";
		if($course_id!='')
		$sql .= " AND TYPING_COURSE_CODE !='$course_id'";
		$res = parent::execQuery($sql);
		if($res && $res->num_rows>0)
		{
			return false;
	...
	public function list_added_courses_typing($inst_id)
	{
		$data = '';
		$sql= "SELECT A.*,A.TYPING_COURSE_FEES as EXAM_FEES,A.TYPING_COURSE_NAME AS COURSE_NAME_MODIFY, B.INSTITUTE_COURSE_ID, B.ACTIVE AS STATUS, B.COURSE_FEES AS INSTITUTE_COURSE_FEES,B.PLAN_ID, B.PLAN_FEES, B.MINIMUM_FEES FROM  courses_typing A LEFT JOIN institute_courses B ON A.TYPING_COURSE_ID=B.TYPING_COURSE_ID  WHERE B.DELETE_FLAG=0  AND B.COURSE_TYPE=1 AND B.INSTITUTE_ID='$inst_id' ";
		//echo $sql;		
		$res = parent:: execQuery($sql);
		if($res && $res->num_rows>0)
		{
	...
	public function list_added_courses_single_typing($inst_course_id)
	{
		$data = '';
		$sql= "SELECT A.*,A.TYPING_COURSE_FEES as EXAM_FEES,A.TYPING_COURSE_NAME AS COURSE_NAME_MODIFY, B.INSTITUTE_COURSE_ID, B.ACTIVE AS STATUS, B.COURSE_FEES AS INSTITUTE_COURSE_FEES, B.PLAN_FEES ,  B.MINIMUM_FEES FROM  courses_typing A LEFT JOIN institute_courses B ON A.TYPING_COURSE_ID=B.TYPING_COURSE_ID  WHERE B.DELETE_FLAG=0  AND B.COURSE_TYPE=1 AND B.INSTITUTE_COURSE_ID='$inst_course_id'";	
		//echo $sql; 	
		$res = parent:: execQuery($sql);
		if($res && $res->num_rows>0)
		{
	...
	public function list_notadded_courses_typing($inst_id)
	{
		$data = '';
		$search ='';
		$where = '';
		...
		$sql = "SELECT A.TYPING_COURSE_ID FROM institute_courses A WHERE A.INSTITUTE_ID='$inst_id' AND A.DELETE_FLAG=0";		
		$res = parent:: execQuery($sql);
		...
		if($res && $res->num_rows>0)
		{
			...
			{
				$TYPING_COURSE_ID = $resdata['TYPING_COURSE_ID'];
				$search .= "'$TYPING_COURSE_ID',";
			}
			$search = rtrim($search,",");
		}
		$where= '';
		if($search!='')
		{
			$where = " AND A.TYPING_COURSE_ID NOT IN($search) ";
		}
	
		 $sql2 = "SELECT A.*,A.TYPING_COURSE_NAME AS COURSE_NAME_MODIFY,B.COURSE_FEES as PLAN_FEES,B.PLAN_ID FROM  courses_typing A LEFT JOIN course_typing_plan_fees B ON A.TYPING_COURSE_ID = B.TYPING_COURSE_ID WHERE  B.PLAN_ID='$plan_id' AND A.DELETE_FLAG=0 AND A.ACTIVE=1 $where";
		//echo $sql2;
	...
	{
		$sql = "SELECT * FROM institute_courses WHERE TYPING_COURSE_ID='$course_id' AND INSTITUTE_ID='$inst_id' AND COURSE_TYPE='$course_type' AND DELETE_FLAG=0";
		...
		if (!empty($errors)) {
			  ...
			  if(isset($errors['course']))
				...
				foreach($course_subject_id as $value)
				{
					...
					$tabFields = "(INSTITUTE_SUBJECT_ID,INSTITUTE_ID,TYPING_COURSE_ID,COURSE_SUBJECT_ID,SUBJECT_DETAILS,POSITION,ACTIVE,DELETE_FLAG,CREATED_BY,CREATED_ON)";
	...
	public function list_added_courses_subject_typing($inst_course_id)
	{
		$data = '';
		$sql= "SELECT A.* FROM  courses_typing_subjects A WHERE A.DELETE_FLAG=0  AND A.TYPING_COURSE_ID='$inst_course_id'";		
		$res = parent:: execQuery($sql);
		if($res && $res->num_rows>0)
		{
			$data = $res;
	...
	{
		$data = '';
		$sql= "SELECT A.INSTITUTE_SUBJECT_ID,A.TYPING_COURSE_ID,A.COURSE_SUBJECT_ID as SUBJECT_ID ,get_subject_title_typing(A.COURSE_SUBJECT_ID) as SUBJECT_NAME,get_subject_speed_typing(A.COURSE_SUBJECT_ID) as SPEED_NAME ,A.SUBJECT_DETAILS,A.POSITION, B.* FROM  institute_course_subjects A RIGHT JOIN course_typing_exam_structure B ON A.COURSE_SUBJECT_ID = B.COURSE_SUBJECT_ID WHERE A.DELETE_FLAG=0 AND B.DELETE_FLAG=0 AND A.INSTITUTE_ID='$inst_id'  AND A.TYPING_COURSE_ID='$inst_course_id'";	
		//echo $sql; 
		$res = parent:: execQuery($sql);
		if($res && $res->num_rows>0)
		{
		...
		if (!empty($errors)) {
			  ...
			  if(isset($errors['course']))
				...
				foreach($course as $value)
				{
					...
					$sql ="SELECT institute_details.PLAN_ID, course_typing_plan_fees.COURSE_FEES AS PLAN_FEES FROM institute_details inner join course_typing_plan_fees on institute_details.PLAN_ID=course_typing_plan_fees.PLAN_ID WHERE institute_details.INSTITUTE_ID=$institute_id AND course_typing_plan_fees.TYPING_COURSE_ID= $value LIMIT 0,1";
					...
					if($res && $res->num_rows>0)
					{
						...
						{
							$tableName = "institute_courses";
							$tabFields = "(INSTITUTE_COURSE_ID,INSTITUTE_ID,TYPING_COURSE_ID,COURSE_TYPE,PLAN_ID,PLAN_FEES,EXAM_FEES,COURSE_FEES,CREATED_BY,CREATED_ON,CREATED_ON_IP,MINIMUM_FEES)";
	...
	public function list_student_exam_results_typing($id='', $stud_id='', $inst_id='',$stud_course_id='', $cond='')
	{
		$data = '';
		 $sql= "SELECT A.*,C.REQUEST_STATUS AS REQUEST_STATUS,DATE_FORMAT(C.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS ORDER_DATE,DATE_FORMAT(D.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS APPROVE_DATE,get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE FROM course_typing_exam_result_final A LEFT JOIN certificate_order_requests C ON C.EXAM_RESULT_FINAL_ID = A.EXAM_RESULT_FINAL_ID LEFT JOIN certificate_requests D ON D.EXAM_RESULT_FINAL_ID = A.EXAM_RESULT_FINAL_ID WHERE A.DELETE_FLAG=0 ";
		if($id!='')
		{

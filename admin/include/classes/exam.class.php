	...
	*/
	public function add_exam()
	{
		...
		if (! empty($errors)) {
			...
			$tabFields 	= "(EXAM_ID, COURSE_ID, EXAM_TITLE, TOTAL_MARKS,TOTAL_QUESTIONS,MARKS_PER_QUE,PASSING_MARKS,EXAM_TIME,EXAM_MODE_TYPE,SHOW_RESULT,DEMO_TEST, ACTIVE,CREATED_BY, CREATED_ON)";
			$insertVals	= "(NULL, '$courseid', '$examname', '$totalmarks','$totalque','$markperque','$passingmarks','$examtime', '$exam_mode','$showresult', '$demotest', '$status','$created_by',NOW())";
	...
	*/
	public function update_exam($exam_id)
	{
		...
		if (!empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "exam_structure";
			$setValues 	= "COURSE_ID='$courseid', EXAM_TITLE=UPPER('$examname'), TOTAL_MARKS='$totalmarks',TOTAL_QUESTIONS='$totalque',MARKS_PER_QUE='$markperque', PASSING_MARKS='$passingmarks', EXAM_TIME='$examtime',EXAM_MODE_TYPE='$exam_mode', SHOW_RESULT='$showresult',DEMO_TEST='$demotest', ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW()";
	...
	public function list_exams($exam_id = '', $condition = '')
	{
		$data = '';
		$sql = "SELECT A.*, get_course_title_modify(A.COURSE_ID) AS COURSE_NAME_MODIFY, (SELECT COURSE_CODE FROM courses WHERE COURSE_ID=A.COURSE_ID) AS COURSE_CODE FROM  exam_structure A WHERE A.DELETE_FLAG=0 ";
	...
	{
		$sql = "UPDATE exam_structure SET DELETE_FLAG=1, UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW() WHERE EXAM_ID='$exam_id'";
		$res = parent::execQuery($sql);
	...
	/* list question bank */
	public function list_quetion_bank($que_id = '', $condition = '')
	{
		$data = '';
		$sql = "SELECT A.*,(SELECT COUNT(*) FROM exam_question_bank B WHERE B.QUEBANK_ID=A.QUEBANK_ID AND B.DELETE_FLAG=0) AS TOTAL_QUE , (SELECT COURSE_CODE FROM courses WHERE COURSE_ID=A.COURSE_ID) AS COURSE_CODE,  DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i %p') AS CREATED_DATE, (SELECT COURSE_CODE FROM courses ac WHERE ac.COURSE_ID=A.COURSE_ID) AS COURSE_CODE FROM  exam_question_bank_master A WHERE A.DELETE_FLAG=0 ";
		if ($que_id != '') {
	...
	/* view question bank */
	public function view_quetion_bank($quebank_id = '', $cond = '')
	{
		$data = '';
		$sql = "SELECT * FROM exam_question_bank WHERE DELETE_FLAG=0 ";
		if ($quebank_id != '') {
	...
	public function add_quebank()
	{
		...
		$file1 = $_FILES["quebankfile"]["tmp_name"];
		$file2 = $_FILES["quebankfilehindi"]["tmp_name"];

		/* check validations */
		if ($courseid == '') $errors['courseid'] = 'Course code is required!';
		if ($examname == '') $errors['examname'] = 'Exam name is required!';

		//if ($quebankfile=='') $errors['quebankfile'] = 'English CSV file is required!';

		//if ($quebankfilehindi=='') $errors['quebankfilehindi'] = 'Hindi CSV file is required!';

		if ($quebankfile != '') {
			$allowed_ext = array('csv', 'xls');
			$extension = pathinfo($quebankfile, PATHINFO_EXTENSION);
			if (!in_array($extension, $allowed_ext)) {
				$errors['quebankfile'] = 'Invalid file format! Please select valid csv, xls file.';
			}
		}
		...
		if (! empty($errors)) {
			...
			if ($quebankfile != '' || $quebankfilehindi != '') {
				...
				$tabFields 	= "(QUEBANK_ID, COURSE_ID, EXAM_NAME, FILE_NAME,HFILE_NAME,IMG_ZIP_FILE, ACTIVE,CREATED_BY, CREATED_ON)";
				$insertVals	= "(NULL, '$courseid', '$examname', '$file_name','$file_name1','$img_zip_file_name','$status','$created_by',NOW())";
				$insertSql	= parent::insertData($tableName, $tabFields, $insertVals);
				$exSql = parent::execQuery($insertSql);
				...
				if ($exSql) {
					...
					$courseImgPathFile 	= 	QUEBANK_PATH . '/' . $last_insert_id . '/English/' . $file_name;
					@mkdir(QUEBANK_PATH . '/' . $last_insert_id . '/English/', 0777, true);
					...
					if ($quebankimgs != '') {
						$zipImgPath 	= 	QUEBANK_PATH . '/' . $last_insert_id . '/';
					...
					if (file_exists($courseImgPathFile)) {
						$objPHPExcel = PHPExcel_IOFactory::load($courseImgPathFile);
						$tableName1 	= "exam_question_bank";
						foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {

							// error_log(print_r('for each ',true));
							$highestRow = $worksheet->getHighestRow();
							for ($row = 2; $row <= $highestRow; $row++) {

								$QUESTION 	= htmlentities($worksheet->getCellByColumnAndRow(0, $row)->getValue(), ENT_QUOTES);
								$IMAGE 		= htmlentities($worksheet->getCellByColumnAndRow(1, $row)->getValue(), ENT_QUOTES);
								$OPTION_A 	= htmlentities($worksheet->getCellByColumnAndRow(2, $row)->getValue(), ENT_QUOTES);
								$OPTION_B 	= htmlentities($worksheet->getCellByColumnAndRow(3, $row)->getValue(), ENT_QUOTES);
								$OPTION_C 	= htmlentities($worksheet->getCellByColumnAndRow(4, $row)->getValue(), ENT_QUOTES);
								$OPTION_D 	= htmlentities($worksheet->getCellByColumnAndRow(5, $row)->getValue(), ENT_QUOTES);
								$CORRECT_ANS = htmlentities($worksheet->getCellByColumnAndRow(6, $row)->getValue(), ENT_QUOTES);

								$tabFields1 	= "(QUESTION_ID,QUEBANK_ID,COURSE_ID,QUESTION,IMAGE, OPTION_A, OPTION_B, OPTION_C,OPTION_D,CORRECT_ANS, CREATED_BY, CREATED_ON,LANG_ID)";
								$insertVals1	= "(NULL,'$last_insert_id','$courseid', '$QUESTION','$IMAGE','$OPTION_A','$OPTION_B', '$OPTION_C', '$OPTION_D','$CORRECT_ANS', '$created_by',NOW(),'1')";
					...
					if (file_exists($courseImgPathFile1)) {
						...
						$objPHPExcel = PHPExcel_IOFactory::load($courseImgPathFile1);
						$tableName1 	= "exam_question_bank";
						foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
							...
							for ($row = 2; $row <= $highestRow; $row++) {
								...
								$tabFields1 	= "(QUESTION_ID,QUEBANK_ID,COURSE_ID,QUESTION,IMAGE, OPTION_A, OPTION_B, OPTION_C,OPTION_D,CORRECT_ANS, CREATED_BY, CREATED_ON,LANG_ID)";
								$insertVals1	= "(NULL,'$last_insert_id','$courseid', '$QUESTION','$IMAGE','$OPTION_A','$OPTION_B', '$OPTION_C', '$OPTION_D','$CORRECT_ANS', '$created_by',NOW(),'2')";
								$insertSql1		= parent::insertData($tableName1, $tabFields1, $insertVals1);
								$exSql1 		= parent::execQuery($insertSql1);
							}
						}
...
*/
	...
	public function delete_que_bank($quebank_id)
	{
		...
		$sql = "DELETE FROM exam_question_bank_master WHERE QUEBANK_ID='$quebank_id'";
		$sql2 = "DELETE FROM exam_question_bank WHERE QUEBANK_ID='$quebank_id'";
		$res = parent::execQuery($sql);
	...
	/* delete question bank */
	public function empty_que_bank($quebank_id)
	{
		$sql2 = "DELETE FROM exam_question_bank WHERE QUEBANK_ID='$quebank_id'";
		$res2 = parent::execQuery($sql2);
	...
	public function get_question_detail($question_id, $que_bank = '')
	{
		$data = '';
		$sql = "SELECT *,DATE_FORMAT(CREATED_ON, '%d-%m-%Y %h:%i %p') AS CREATED_DATE FROM exam_question_bank WHERE QUESTION_ID='$question_id'";
		if ($que_bank != '')
	...
	public function edit_question($question_id)
	{
		...
		$lang 	= parent::test(isset($_POST['lang']) ? $_POST['lang'] : '');
		...
		if (! empty($errors)) {
			...
			$setValues 	= "QUESTION='$question', OPTION_A='$opt1',OPTION_B='$opt2', OPTION_C='$opt3',OPTION_D='$opt4',CORRECT_ANS='$correctans',LANG_ID='$lang', ACTIVE='$status',UPDATED_BY='$updated_by', UPDATED_ON=NOW(),UPDATED_BY_IP='$ip_address'";
			if ($queimg != '') {
	...
	public function add_question($quebank_id)
	{
		$errors = array();  // array to hold validation errors
		$data = array();        // array to pass back data
		...
		if ($lang == '') $errors['lang'] = 'Please select language !';
		...
		if (! empty($errors)) {
			...
			parent::start_transaction();
			$tableName 	= "exam_question_bank";
			$tabFields = "(QUEBANK_ID,COURSE_ID,QUESTION,OPTION_A,OPTION_B,OPTION_C,OPTION_D,CORRECT_ANS,ACTIVE,CREATED_BY,CREATED_ON,CREATED_BY_IP,LANG_ID";
			$insertValues = "('$quebank_id','$course_id','$question', '$opt1', '$opt2', '$opt3', '$opt4', '$correctans', '$status', '$updated_by', NOW(), '$ip_address','$lang'";
			if ($queimg != '') {
				$ext 		= pathinfo($_FILES["queimg"]["name"], PATHINFO_EXTENSION);
				$randno 	= mt_rand(0, 123456789);
				$file_name  = 'A' . $randno . '.' . $ext;
	...
	{
		$sql = "DELETE FROM exam_question_bank WHERE QUESTION_ID='$question_id'";
		$res = parent::execQuery($sql);
	...
	/* change exam status */
	public function changeExamStatus($exam_id, $flag)
	{
		$sql = "UPDATE exam_structure SET ACTIVE='$flag',UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW(),UPDATED_ON_IP='" . $_SESSION['ip_address'] . "' WHERE EXAM_ID='$exam_id'";
		$res = parent::execQuery($sql);
	...
	/* change exam status */
	public function changeExamResultDisp($exam_id, $flag)
	{
		$sql = "UPDATE exam_structure SET SHOW_RESULT='$flag',UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW(),UPDATED_ON_IP='" . $_SESSION['ip_address'] . "' WHERE EXAM_ID='$exam_id'";
		$res = parent::execQuery($sql);
	...
	public function changeExamDemoStatus($exam_id, $flag)
	{
		$sql = "UPDATE exam_structure SET DEMO_TEST='$flag',UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW(),UPDATED_ON_IP='" . $_SESSION['ip_address'] . "' WHERE EXAM_ID='$exam_id'";
		$res = parent::execQuery($sql);
	...
	{
		$result = '';
		$sql = "SELECT COURSE_NAME,get_course_title_modify(COURSE_ID) as COURSE_NAME_MODIFY FROM courses WHERE COURSE_ID='$course_id' LIMIT 0,1";
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$data = $res->fetch_assoc();
			$result = $data['COURSE_NAME_MODIFY'];
		}
		return $result;
	}
	...
	public function list_student_exam_results($id = '', $stud_id = '', $inst_id = '', $stud_course_id = '', $cond = '')
	{
		$data = '';
		$sql = "SELECT A.*,C.REQUEST_STATUS AS REQUEST_STATUS,DATE_FORMAT(C.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS ORDER_DATE,DATE_FORMAT(D.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS APPROVE_DATE,get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE, (SELECT B.EXAM_TYPE FROM exam_types_master B WHERE B.EXAM_TYPE_ID=A.EXAM_TYPE) AS EXAM_TYPE_NAME FROM exam_result A LEFT JOIN certificate_order_requests C ON C.EXAM_RESULT_ID = A.EXAM_RESULT_ID LEFT JOIN certificate_requests D ON D.EXAM_RESULT_ID = A.EXAM_RESULT_ID WHERE A.DELETE_FLAG=0 ";
		if ($id != '') {
			$sql .= " AND A.EXAM_RESULT_ID='$id' ";
		}
		if ($inst_id != '') {
	...
	public function delete_stud_exam_result($exam_result_id)
	{
		$res = '';
		$tableName		= "exam_result";
		$setValues 		= " DELETE_FLAG='1', ACTIVE='0', UPDATED_BY='" . $_SESSION['user_fullname'] . "', UPDATED_ON=NOW(), UPDATED_ON_IP='" . $_SESSION['ip_address'] . "'";
	...
	/*public function apply_for_certificate_wihtout_pay()
	{
		...
		/*if($checkstud=='' || empty($checkstud) || $checkstud_multisub=='' || empty($checkstud_multisub)) $errors['checkstud'] = 'Please select atleast one student!';
		 ...
		 if (! empty($errors)) {				 
			  ...
			  $data['message']  = $message;			 
				...
				if(is_array($checkstud) && count($checkstud)>0)
				{
					...
					$insertSql1 = "INSERT INTO $tableName1 $tabFields1 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES),'$created_by',NOW(),'$ip_address' FROM exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr) ";
					$exSql1		= parent::execQuery($insertSql1);
					$last_id 	= parent::last_id();
				...
				if(is_array($checkstud_multisub) && count($checkstud_multisub)>0)
				{
					$resultidstr_multisub = rtrim(implode(",", $checkstud_multisub),",");
					$failedArr_multisub = array();
					
					$tableName1_multisub = 'certificate_requests_master';
					$tabFields1_multisub = "(INSTITUTE_ID,TOTAL_EXAM_FEES, CREATED_BY, CREATED_ON, CREATED_ON_IP)";					
				
					$insertSql1_multisub = "INSERT INTO $tableName1_multisub $tabFields1_multisub 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES),'$created_by',NOW(),'$ip_address' FROM multi_sub_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID IN ($resultidstr_multisub) ";
					
					$exSql1_multisub	= parent::execQuery($insertSql1_multisub);
					$last_id_multisub 	= parent::last_id();
					
					foreach($checkstud_multisub as $examresultid_multisub)
					{
						$tableName_multisub = 'certificate_requests';
						$tabFields_multisub = "(CERTIFICATE_REQUEST_MASTER_ID,EXAM_RESULT_FINAL_ID, STUDENT_ID, INSTITUTE_ID,MULTI_SUB_COURSE_ID,GRADE,MARKS_PER, RESULT_STATUS,EXAM_TITLE,EXAM_FEES, REQUEST_STATUS, REQUEST_RESPONSE_MSG, CREATED_BY, CREATED_ON, CREATED_ON_IP)";					
						
						$insertSql_multisub = "INSERT INTO $tableName_multisub $tabFields_multisub 
										SELECT '$last_id_multisub',A.EXAM_RESULT_FINAL_ID, A.STUDENT_ID, A.INSTITUTE_ID,B.MULTI_SUB_COURSE_ID,A.GRADE,A.MARKS_PER, A.RESULT_STATUS,A.EXAM_TITLE,B.PLAN_FEES AS EXAM_FEES,1,'','$created_by',NOW(),'$ip_address'  FROM multi_sub_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID = '$examresultid_multisub'; ";	
						
						
						$updSql_multisub = " UPDATE multi_sub_exam_result_final SET APPLY_FOR_CERTIFICATE=1 WHERE EXAM_RESULT_FINAL_ID='$examresultid_multisub';";
					
						$exSql_multisub		= parent::execQuery($insertSql_multisub);
						$exSql2_multisub		= parent::execQuery($updSql_multisub);
						if(!$exSql_multisub && parent::rows_affected()<=0)
							array_push($failedArr_multisub,$examresultid_multisub);
				...
				if(empty($failedArr) && empty($failedArr_multisub))
					{
						parent::commit();
						$data['success'] = true;
						$data['message'] = 'Success! Students has been aplied for certificates successfully!';
	...
	public function apply_for_certificate_wihtout_pay()
	{
		...
		if (! empty($errors)) {
			$data['errors']  	= $errors;
			$message  		= isset($errors['checkstud']) ? $errors['checkstud'] : '';
			$message  		= isset($errors['checkstud_multisub']) ? $errors['checkstud_multisub'] : '';
			$message  		= isset($errors['checkstud_typing']) ? $errors['checkstud_typing'] : '';
			$data['message']  = $message;
		} else {
			if (is_array($checkstud) && count($checkstud) > 0) {
				$resultidstr = rtrim(implode(",", $checkstud), ",");
		...
		if (is_array($checkstud_typing) && count($checkstud_typing) > 0) {
			$resultidstr_typing = rtrim(implode(",", $checkstud_typing), ",");
			$failedArr_typing = array();

			$tableName1_typing = 'certificate_requests_master';
			$tabFields1_typing = "(INSTITUTE_ID,TOTAL_EXAM_FEES,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

			$insertSql1_typing = "INSERT INTO $tableName1_typing $tabFields1_typing 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES),'$created_by',NOW(),'$ip_address' FROM course_typing_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID IN ($resultidstr_typing) ";

			$exSql1_typing	= parent::execQuery($insertSql1_typing);
			$last_id_typing 	= parent::last_id();

			foreach ($checkstud_typing as $examresultid_typing) {
				$tableName_typing = 'certificate_requests';
				$tabFields_typing = "(CERTIFICATE_REQUEST_MASTER_ID,EXAM_RESULT_TYPING_ID, STUDENT_ID, INSTITUTE_ID,TYPING_COURSE_ID,GRADE,MARKS_PER, RESULT_STATUS,EXAM_TITLE,EXAM_FEES, REQUEST_STATUS, REQUEST_RESPONSE_MSG,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

				$insertSql_typing = "INSERT INTO $tableName_typing $tabFields_typing 
										SELECT '$last_id_typing',A.EXAM_RESULT_FINAL_ID, A.STUDENT_ID, A.INSTITUTE_ID,B.TYPING_COURSE_ID,A.GRADE,A.MARKS_PER, A.RESULT_STATUS,A.EXAM_TITLE,B.PLAN_FEES AS EXAM_FEES,1,'','$created_by',NOW(),'$ip_address'  FROM course_typing_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID = '$examresultid_typing'; ";


				$updSql_typing = " UPDATE course_typing_exam_result_final SET APPLY_FOR_CERTIFICATE=1 WHERE EXAM_RESULT_FINAL_ID='$examresultid_typing';";

				$exSql_typing		= parent::execQuery($insertSql_typing);
				$exSql2_typing		= parent::execQuery($updSql_typing);
				if (!$exSql_typing && parent::rows_affected() <= 0)
					array_push($failedArr_typing, $examresultid_typing);
			}
		}

		if (empty($failedArr) && empty($failedArr_typing)) {
			parent::commit();
			$data['success'] = true;
			$data['message'] = 'Success! Students has been aplied for certificates successfully!';
		} else {
	...
	public function order_for_certificate()
	{
		$errors 	= array();  // array to hold validation errors
		$data 		= array();
		$data['success'] = false;		// array to pass back data		
		...
		$totalToPay = 0;
		$wallet_id = '';
		$res = parent::get_courier_wallet('', $institute_id, 8);
		if ($res != '') {
			$data1 = $res->fetch_assoc();
			$walletBal = $data1['TOTAL_BALANCE'];
			$wallet_id = $data1['WALLET_ID'];
		} else {
			$errors['checkstud'] = "Sorry! Your Courier wallet is empty!";
			$errors['checkstud_multisub'] = "Sorry! Your Courier wallet is empty!";
			$errors['checkstud_typing'] = "Sorry! Your Courier wallet is empty!";
		}
		...
		if (is_array($checkstud) && count($checkstud) > 0) {
			...
			$sql2 = "SELECT DISTINCT A.INSTITUTE_ID,SUM((SELECT C.COURSE_FEES FROM courses C WHERE C.COURSE_ID=B.COURSE_ID))+120 AS TOTAL_FEES FROM exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr)";
			...
			if ($res2 && $res2->num_rows > 0) {
				...
				$totalToPay = 120;
				if ($totalToPay > $walletBal)
					$errors['checkstud'] = "Sorry! Your total bill is <strong>Rs. $totalToPay</strong>.  You have only <strong>Rs. $walletBal</strong> available in your courier wallet! You need more <strong> Rs. " . ($totalToPay - $walletBal) . "</strong> to order the certificates.<br> Please rechrage your courier wallet. <a href='courier-pay-online'>Recharge Now!</a>";
			}
		}

		if (is_array($checkstud_multisub) && count($checkstud_multisub) > 0) {
			$resultidstr_multisub1 = rtrim(implode(",", $checkstud_multisub), ",");
			/* get total fees */
			//$sql2 = "SELECT DISTINCT A.INSTITUTE_ID, (SUM(B.EXAM_FEES)+100) AS TOTAL_FEES  FROM exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr)";

			$sql2 = "SELECT DISTINCT A.INSTITUTE_ID,SUM((SELECT C.MULTI_SUB_COURSE_FEES FROM multi_sub_courses
 C WHERE C.MULTI_SUB_COURSE_ID=B.MULTI_SUB_COURSE_ID))+120 AS TOTAL_FEES FROM exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr_multisub1)";
			...
			if ($res2 && $res2->num_rows > 0) {
				...
				if ($totalToPay > $walletBal)
					$errors['checkstud'] = "Sorry! Your total bill is <strong>Rs. $totalToPay</strong>.  You have only <strong>Rs. $walletBal</strong> available in your courier wallet! You need more <strong> Rs. " . ($totalToPay - $walletBal) . "</strong> to order the certificates.<br> Please rechrage your courier wallet. <a href='courier-pay-online'>Recharge Now!</a>";
				error_log(print_r("and here", true));
			}
		}
		...
		if (is_array($checkstud_typing) && count($checkstud_typing) > 0) {
			...
			$sql2 = "SELECT DISTINCT A.INSTITUTE_ID,SUM((SELECT C.TYPING_COURSE_FEES FROM courses_typing C WHERE C.TYPING_COURSE_ID =B.TYPING_COURSE_ID))+120 AS TOTAL_FEES FROM course_typing_exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr_typing)";
		...
		if (! empty($errors)) {
			$data['errors']  	= $errors;
			$message  		= isset($errors['checkstud']) ? $errors['checkstud'] : '';
			$message1  		= isset($errors['checkstud_multisub']) ? $errors['checkstud_multisub'] : '';
			$message2  		= isset($errors['checkstud_typing']) ? $errors['checkstud_typing'] : '';
			if ($message != '') {
				$data['message']  = $message;
			} elseif ($message1 != '') {
			...
			$sqlwallet = "UPDATE courier_wallet SET TOTAL_BALANCE= TOTAL_BALANCE - 120, UPDATED_BY='$created_by', UPDATED_ON=NOW(),UPDATED_ON_IP='$ip_address' WHERE WALLET_ID='$wallet_id'";
			$reswallet = parent::execQuery($sqlwallet);

			if ($wallet_id != '') {
				$user_info 	= $this->get_user_info($institute_id, 8);
				$NAME 		= $user_info['NAME'];
				$MOBILE 	= $user_info['MOBILE'];
				$EMAIL 		= $user_info['EMAIL'];

				$tableName3 	= "offline_payments";
				$tabFields3 	= "(PAYMENT_ID, TRANSACTION_NO,TRANSACTION_TYPE,USER_ID,USER_ROLE,USER_FULLNAME,USER_EMAIL,USER_MOBILE,PAYMENT_AMOUNT,PAYMENT_MODE,PAYMENT_DATE,PAYMENT_STATUS,PAYMENT_REMARK,COURIER_WALLET_ID,ACTIVE,CREATED_BY, CREATED_ON,CREATED_BY_IP,COURIER_WALLET_PAYMENT)";
				$insertVals3	= "(NULL, get_payment_transaction_id_admin(), 'DEBIT','$institute_id','8', '$NAME','$EMAIL','$MOBILE','$totalToPay','OFFLINE',NOW(), 'success', 'Certificate Ordered','$wallet_id', '1','$created_by',NOW(),'$ip_address','1')";
				$insertSql3	= parent::insertData($tableName3, $tabFields3, $insertVals3);
				$exSql3		= parent::execQuery($insertSql3);
				...
				$tabFields2 = "(RECIEPT_NO, INSTITUTE_ID, CERTIFICATE_REQUEST_MASTER_ID, TOTAL_EXAM_FEES,TOTAL_EXAM_FEES_RECIEVED,TOTAL_EXAM_FEES_BALANCE,PAYMENT_DATE, CREATED_BY, CREATED_ON, CREATED_ON_IP)";
				$insertSql2 = "INSERT INTO $tableName2 $tabFields2 
								SELECT generate_admin_reciept_num(), INSTITUTE_ID, CERTIFICATE_REQUEST_MASTER_ID, TOTAL_EXAM_FEES,0,TOTAL_EXAM_FEES, NOW(), '$created_by', NOW(), '$ip_address' FROM certificate_requests_master WHERE  CERTIFICATE_REQUEST_MASTER_ID='$last_id_multisub'";
				$exSql2		= parent::execQuery($insertSql2);
			}

			if (is_array($checkstud) && count($checkstud) > 0) {
				/*$count_checkstud = count($checkstud);
					$temp = print_r($checkstud);
					error_log(print_r("$temp", true));
					error_log(print_r("$count_checkstud", true));*/

				$resultidstr = rtrim(implode(",", $checkstud), ",");
				$failedArr = array();

				$tableName1 = 'certificate_order_requests_master';
				$tabFields1 = "(INSTITUTE_ID,TOTAL_EXAM_FEES,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

				$insertSql1 = "INSERT INTO $tableName1 $tabFields1 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES)+120,'$created_by',NOW(),'$ip_address' FROM exam_result A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_ID IN ($resultidstr) ";
				$exSql1		= parent::execQuery($insertSql1);
				$last_id 	= parent::last_id();

				if ($wallet_id != '') {
					foreach ($checkstud as $examresultid) {
						$tableName = 'certificate_order_requests';
						$tabFields = "(CERTIFICATE_REQUEST_MASTER_ID,EXAM_RESULT_ID, STUDENT_ID, INSTITUTE_ID,COURSE_ID,EXAM_TITLE,OBJECTIVE_MARKS,PRACTICAL_MARKS,SUBJECT,GRADE,MARKS_PER, RESULT_STATUS, EXAM_TYPE,EXAM_FEES, REQUEST_STATUS, REQUEST_RESPONSE_MSG,CREATED_BY, CREATED_ON, CREATED_ON_IP)";
						...
						$updSql = " UPDATE exam_result SET ORDER_FOR_CERTIFICATE=1 WHERE EXAM_RESULT_ID='$examresultid';";
			...
			if (is_array($checkstud_multisub) && count($checkstud_multisub) > 0) {
				...
				$tableName1_multisub = 'certificate_order_requests_master';
				$tabFields1_multisub = "(INSTITUTE_ID,TOTAL_EXAM_FEES,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

				$insertSql1_multisub = "INSERT INTO $tableName1_multisub $tabFields1_multisub 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES)+120,'$created_by',NOW(),'$ip_address' FROM multi_sub_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID IN ($resultidstr_multisub) ";
				...
				if ($wallet_id != '') {
					foreach ($checkstud_multisub as $examresultid_multisub) {
						...
						$tableName_multisub = 'certificate_order_requests';
						$tabFields_multisub = "(CERTIFICATE_REQUEST_MASTER_ID,EXAM_RESULT_FINAL_ID, STUDENT_ID, INSTITUTE_ID,MULTI_SUB_COURSE_ID,GRADE,MARKS_PER, RESULT_STATUS,EXAM_TITLE,EXAM_FEES, REQUEST_STATUS, REQUEST_RESPONSE_MSG,CREATED_BY, CREATED_ON, CREATED_ON_IP)";
						...
						$updSql_multisub = " UPDATE multi_sub_exam_result_final SET ORDER_FOR_CERTIFICATE=1 WHERE EXAM_RESULT_FINAL_ID='$examresultid_multisub';";
			...
			if (is_array($checkstud_typing) && count($checkstud_typing) > 0) {
				...
				$tableName1_typing = 'certificate_order_requests_master';
				$tabFields1_typing = "(INSTITUTE_ID,TOTAL_EXAM_FEES,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

				$insertSql1_typing = "INSERT INTO $tableName1_typing $tabFields1_typing 
										SELECT DISTINCT A.INSTITUTE_ID, SUM(B.PLAN_FEES)+120,'$created_by',NOW(),'$ip_address' FROM course_typing_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID IN ($resultidstr_typing) ";
				...
				if ($wallet_id != '') {
					foreach ($checkstud_typing as $examresultid_typing) {
						$tableName_typing = 'certificate_order_requests';
						$tabFields_typing = "(CERTIFICATE_REQUEST_MASTER_ID,EXAM_RESULT_FINAL_ID_TYPING, STUDENT_ID, INSTITUTE_ID,TYPING_COURSE_ID,GRADE,MARKS_PER, RESULT_STATUS,EXAM_TITLE,EXAM_FEES, REQUEST_STATUS, REQUEST_RESPONSE_MSG,CREATED_BY, CREATED_ON, CREATED_ON_IP)";

						$insertSql_typing = "INSERT INTO $tableName_typing $tabFields_typing
										SELECT '$last_id_typing',A.EXAM_RESULT_FINAL_ID, A.STUDENT_ID, A.INSTITUTE_ID,B.TYPING_COURSE_ID,A.GRADE,A.MARKS_PER, A.RESULT_STATUS,A.EXAM_TITLE,B.PLAN_FEES AS EXAM_FEES,1,'','$created_by',NOW(),'$ip_address'  FROM course_typing_exam_result_final A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.EXAM_RESULT_FINAL_ID = '$examresultid_typing'; ";


						$updSql_typing = " UPDATE  course_typing_exam_result_final SET ORDER_FOR_CERTIFICATE=1 WHERE EXAM_RESULT_FINAL_ID='$examresultid_typing';";
			...
			if (empty($failedArr) && empty($failedArr_multisub) && empty($failedArr_typing)) {
				parent::commit();
				$data['success'] = true;
				$data['message'] = 'Success! Students has been applied for certificates successfully!';
			} else {
				parent::rollback();
				$data['success'] = false;
				$data['message'] = 'Sorry! Application for certificates failed!';
...
INNER JOIN student_enquiry ON certificate_requests.INSTITUTE_ID=student_enquiry.INSTITUTE_ID
	...
	{
		$data = '';
		$sql = "SELECT A.*,get_institute_code(A.INSTITUTE_ID) AS INSTITUTE_CODE,get_student_code(A.STUDENT_ID) AS STUDENT_CODE,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_institute_name(A.INSTITUTE_ID) AS INSTITUTE_NAME, get_student_certificate_name(SD.STUDENT_FNAME,SD.STUDENT_MNAME,SD.STUDENT_LNAME,SD.CERT_MNAME,SD.CERT_LNAME,SD.SONOF) AS STUDENT_NAME,SD.STUDENT_FNAME,SD.STUDENT_MNAME,SD.STUDENT_LNAME,SD.SONOF, SD.STUDENT_MOTHERNAME,SD.CREATED_ON as stud_admission_date, SD.STUDENT_DOB,  DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE,(SELECT B.EXAM_TYPE FROM exam_types_master B WHERE B.EXAM_TYPE_ID=A.EXAM_TYPE) AS EXAM_TYPE_NAME,(SELECT C.REQUEST_STATUS FROM certificate_requests_status_master C WHERE C.REQUEST_STATUS_ID=A.REQUEST_STATUS) AS REQUEST_STATUS_NAME, A.EXAM_FEES AS EXAM_FEES, (SELECT UPPER(D.COURSE_DURATION) FROM courses D WHERE D.COURSE_ID=A.COURSE_ID) AS COURSE_DURATION, get_institute_city(A.INSTITUTE_ID) AS INSTITUTE_CITY,get_course_title_modify(A.COURSE_ID) AS AICPE_COURSE_AWARD,get_course_multi_sub_title_modify(A.MULTI_SUB_COURSE_ID)AS AICPE_COURSE_AWARD1 FROM certificate_requests A INNER JOIN student_details SD ON A.STUDENT_ID = SD.STUDENT_ID WHERE A.DELETE_FLAG=0 ";
		...
		//         SD.STUDENT_CODE,
		//         SD.STUDENT_DOB,
		//         get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, get_stud_sign(A.STUDENT_ID) AS STUDENT_SIGN,  
		//           get_student_certificate_name(SD.STUDENT_FNAME,SD.STUDENT_MNAME,SD.STUDENT_LNAME,SD.CERT_MNAME,SD.CERT_LNAME,SD.SONOF) AS STUDENT_NAME,
		//         SD.STUDENT_FNAME,
		//         SD.STUDENT_MNAME,
		//         SD.STUDENT_LNAME,SD.SONOF,
		//         SD.STUDENT_MOTHERNAME,SD.CREATED_ON as stud_admission_date,

		//         DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE,               
		//         CRSM.REQUEST_STATUS AS REQUEST_STATUS_NAME, 
		//         A.EXAM_FEES AS EXAM_FEES,
		//         get_course_title_modify(A.COURSE_ID) AS AICPE_COURSE_AWARD,get_course_multi_sub_title_modify(A.MULTI_SUB_COURSE_ID)AS AICPE_COURSE_AWARD1
...
$sql= "SELECT A.*,get_institute_code(A.INSTITUTE_ID) AS INSTITUTE_CODE,get_student_code(A.STUDENT_ID) AS STUDENT_CODE,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_institute_name(A.INSTITUTE_ID) AS INSTITUTE_NAME,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE,(SELECT B.EXAM_TYPE FROM exam_types_master B WHERE B.EXAM_TYPE_ID=A.EXAM_TYPE) AS EXAM_TYPE_NAME,(SELECT C.REQUEST_STATUS FROM certificate_requests_status_master C WHERE C.REQUEST_STATUS_ID=A.REQUEST_STATUS) AS REQUEST_STATUS_NAME, (SELECT D.COURSE_FEES FROM courses D WHERE D.COURSE_ID=A.COURSE_ID) AS EXAM_FEES,(SELECT UPPER(D.COURSE_DURATION) FROM courses D WHERE D.COURSE_ID=A.COURSE_ID) AS COURSE_DURATION, get_institute_city(A.INSTITUTE_ID) AS INSTITUTE_CITY,get_course_title_modify(A.COURSE_ID) AS AICPE_COURSE_AWARD FROM certificate_requests A WHERE A.DELETE_FLAG=0 ";
        ...
        $sql = "SELECT 
                ...
                CRSM.REQUEST_STATUS AS REQUEST_STATUS_NAME, 
                A.EXAM_FEES AS EXAM_FEES, UPPER(C.COURSE_DURATION) AS COURSE_DURATION,
                get_institute_city(A.INSTITUTE_ID) AS INSTITUTE_CITY,
	...
	public function list_order_certificates_requests($id = '', $stud_id = '', $inst_id = '', $cond = '')
	{
		$data = '';
		/*$sql= "SELECT A.*,get_institute_code(A.INSTITUTE_ID) AS INSTITUTE_CODE,get_student_code(A.STUDENT_ID) AS STUDENT_CODE,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_institute_name(A.INSTITUTE_ID) AS INSTITUTE_NAME,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE,(SELECT B.EXAM_TYPE FROM exam_types_master B WHERE B.EXAM_TYPE_ID=A.EXAM_TYPE) AS EXAM_TYPE_NAME,(SELECT C.REQUEST_STATUS FROM certificate_requests_status_master C WHERE C.REQUEST_STATUS_ID=A.REQUEST_STATUS) AS REQUEST_STATUS_NAME, (SELECT D.COURSE_FEES FROM courses D WHERE D.COURSE_ID=A.COURSE_ID) AS EXAM_FEES,(SELECT UPPER(D.COURSE_DURATION) FROM courses D WHERE D.COURSE_ID=A.COURSE_ID) AS COURSE_DURATION, get_institute_city(A.INSTITUTE_ID) AS INSTITUTE_CITY,get_course_title_modify(A.COURSE_ID) AS AICPE_COURSE_AWARD FROM certificate_requests A WHERE A.DELETE_FLAG=0 ";
*/
		$sql = "SELECT 
                ...
                DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%i:%p') AS CREATED_DATE,               
                CRSM.REQUEST_STATUS AS REQUEST_STATUS_NAME, 
                A.EXAM_FEES AS EXAM_FEES,(SELECT F.CITY_NAME as city_name FROM city_master F WHERE A.INSTITUTE_ID=F.CITY_ID) AS INSTITUTE_CITY,
                get_course_title_modify(A.COURSE_ID) AS AICPE_COURSE_AWARD,get_course_multi_sub_title_modify(A.MULTI_SUB_COURSE_ID)AS AICPE_COURSE_AWARD1
                FROM certificate_order_requests A
                INNER JOIN student_details SD ON A.STUDENT_ID = SD.STUDENT_ID
                INNER JOIN institute_details ID ON A.INSTITUTE_ID = ID.INSTITUTE_ID               
                INNER JOIN certificate_requests_status_master CRSM ON A.REQUEST_STATUS=CRSM.REQUEST_STATUS_ID              
                WHERE A.DELETE_FLAG=0";
...
*/
	...
	public function count_total_question($course_id)
	{
		$total = 0;
		$sql = "SELECT COUNT(*) AS TOTAL_QUE FROM exam_question_bank B WHERE B.COURSE_ID='$course_id' AND B.DELETE_FLAG=0 ";
		$res = parent::execQuery($sql);
		if ($res && $res->num_rows > 0) {
			$data = $res->fetch_assoc();
	...
	public function list_quetion_bank_master($course_id = '', $condition = '')
	{
		$data = '';


		$sql = "SELECT DISTINCT A.COURSE_ID,A.QUEBANK_ID  ,get_course_title_modify(A.COURSE_ID) AS EXAM_NAME, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y %h:%s %p') AS CREATED_DATE,D.COURSE_NAME  FROM  exam_question_bank A  INNER JOIN courses D ON D.COURSE_ID=A.COURSE_ID  WHERE A.DELETE_FLAG=0 ";
	...
	public function add_practical_exam_marks()
	{
		$errors 			= array();  // array to hold validation errors
		$data 				= array();
		$data['success'] 	= false;		// array to pass back data
		...
		$marksobtpract 			= parent::test(isset($_POST['marksobtpract']) ? $_POST['marksobtpract'] : '');
		$subject 			    = parent::test(isset($_POST['subject']) ? $_POST['subject'] : '');
		...
		if ($marks_per == '') $errors['marks_per'] = 'Please enter the marks!';
		if ($marks_per != 0 && $marks_per != '' && !parent::valid_decimal($marks_per)) $errors['marks_per'] = 'Invalid entry! Enter number only!';
		//if($totalincorrect!=0 && $totalincorrect!='' && !parent::valid_decimal($totalincorrect)) $errors['totalincorrect'] = 'Invalid entry! Enter number only!';
		//if($marksobt!=0 && $marksobt!='' && !parent::valid_decimal($marksobt)) $errors['marksobt'] = 'Invalid entry! Enter number only!';
		...
		if (! empty($errors)) {
			...
			$tableName 	= "exam_result";
			$tabFields 	= "(EXAM_RESULT_ID, STUD_COURSE_ID,STUDENT_ID,INSTITUTE_ID,EXAM_ID, 
									INSTITUTE_COURSE_ID, EXAM_TITLE,EXAM_ATTEMPT, EXAM_SECRETE_CODE,EXAM_TOTAL_QUE, EXAM_TOTAL_MARKS,EXAM_MARKS_PER_QUE, EXAM_PASSING_MARKS, CORRECT_ANSWER,INCORRECT_ANSWER,  MARKS_OBTAINED,PRACTICAL_MARKS,SUBJECT,MARKS_PER, RESULT_STATUS, GRADE,EXAM_TYPE, CREATED_BY, CREATED_ON, CREATED_ON_IP)";

			$insertVals	= "(NULL,'$stud_course_detail_id','$student_id','$institute_id','$exam_id','$inst_course_id','$exam_title','1','$exam_secrete_code', '$exam_total_que', '$exam_total_marks','$exam_marks_per_que','$exam_passing_marks','$totalcorrect','$totalincorrect','$marksobt','$marksobtpract','$subject','$marks_per', '$result_status','$grade', '$exam_type','$created_by',NOW(),'$created_on_ip')";
			...
			if ($exSql) {
				...
				/*
						...
						*/
				/* update the exam status to appeared*/
				$sqlUpd 	= "UPDATE student_course_details SET EXAM_STATUS='3' WHERE STUD_COURSE_DETAIL_ID='$stud_course_detail_id'";
				parent::execQuery($sqlUpd);
				parent::commit();
				$data['success'] = true;
	...
	/*	public function update_practical_exam_marks()
	{
		$errors 			= array();  // array to hold validation errors
		$data 				= array();  
		// array to pass back data
	...
	public function update_practical_exam_marks()
	{
		...
		if ($marks_per == '') $errors['marks_per'] = 'Please enter the marks!';
		if ($marks_per != 0 && $marks_per != '' && !parent::valid_decimal($marks_per)) $errors['marks_per'] = 'Invalid entry! Enter number only!';
		if ($marksobtpract == '') $errors['marksobtpract'] = 'Please enter the practical  marks!';
		if ($subject == '') $errors['subject'] = 'Please enter !';
		if ($marksobt == '') $errors['marksobt'] = 'Please enter the objective  marks!';
		...
		if (! empty($errors)) {
			...
			/*
				...
				*/
			$tableName		= "exam_result";
			$setValues 		= "CORRECT_ANSWER='$totalcorrect', INCORRECT_ANSWER='$totalincorrect',
				SUBJECT='$subject',MARKS_OBTAINED='$marksobt',PRACTICAL_MARKS='$marksobtpract', MARKS_PER='$marks_per',RESULT_STATUS='$result_status',GRADE='$grade',UPDATED_BY='$created_by', UPDATED_ON=NOW(),UPDATED_ON_IP='$created_on_ip'";
	...
	public function reset_student_exam($stud_course_id)
	{
		...
		if ($exSql && parent::rows_affected() > 0) {
			...
			$tableName21 	= "multi_sub_exam_result";
			$setValues21 	= "DELETE_FLAG='1',APPLY_FOR_CERTIFICATE=0, UPDATED_BY='" . $_SESSION['user_name'] . "', UPDATED_ON=NOW(),UPDATED_ON_IP='" . $_SESSION['ip_address'] . "'";
			$whereClause21	= " WHERE STUD_COURSE_ID='$stud_course_id'";
			$updateSql21		= parent::updateData($tableName21, $setValues21, $whereClause21);
			$exSql21			= parent::execQuery($updateSql21);

			$tableName22 	= "multi_sub_exam_result_final";
			$setValues22 	= "DELETE_FLAG='1',APPLY_FOR_CERTIFICATE=0, UPDATED_BY='" . $_SESSION['user_name'] . "', UPDATED_ON=NOW(),UPDATED_ON_IP='" . $_SESSION['ip_address'] . "'";
			$whereClause22	= " WHERE STUD_COURSE_ID='$stud_course_id'";
			$updateSql22		= parent::updateData($tableName22, $setValues22, $whereClause22);
			$exSql22		= parent::execQuery($updateSql22);
	...
	public function add_student_admission_paid()
	{
		...
		if (is_array($checkstud) && count($checkstud) > 0) {
			...
			$sql2 = "SELECT DISTINCT A.INSTITUTE_ID,SUM(B.PLAN_FEES) AS TOTAL_FEES FROM student_course_details A LEFT JOIN institute_courses B ON A.INSTITUTE_COURSE_ID=B.INSTITUTE_COURSE_ID WHERE A.STUD_COURSE_DETAIL_ID IN ($resultidstr)";
		...
		if (! empty($errors)) {
			...
			if (is_array($checkstud) && count($checkstud) > 0) {
				...
				$sql = "UPDATE student_course_details SET ADMISSION_CONFIRMED='1', OFFLINE_PAYMENT_ID='$payment_id' WHERE STUD_COURSE_DETAIL_ID IN ($resultidstr)";
				$exSql1		= parent::execQuery($sql);
	...
	//list student exam results
	public function list_student_demo_exam_sessions($id = '', $stud_id = '', $cond = '')
	{
		$data = '';
		$sql = "SELECT DISTINCT A.session_id FROM p_exam_attempt A WHERE 1 ";
		if ($id != '') {
			$sql .= " AND A.id='$id' ";
		}
		if ($stud_id != '') {
	...
	public function list_student_demo_exam_results($id = '', $stud_id = '', $session_id = '')
	{
		$data = '';
		$sql = "SELECT COUNT(id) AS CorrectAnswer,A.session_id,B.EXAM_TITLE,B.TOTAL_MARKS,B.TOTAL_QUESTIONS,B.MARKS_PER_QUE,B.PASSING_MARKS,B.EXAM_TIME,B.SHOW_RESULT,B.COURSE_ID FROM p_exam_attempt A LEFT JOIN exam_structure B ON A.exam_id = B.EXAM_ID  WHERE answer_status = 1 ";
		if ($id != '') {
			$sql .= " AND A.id='$id' ";
		}
		if ($stud_id != '') {

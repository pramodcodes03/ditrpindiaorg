 <div class="content-wrapper">
 	<div class="col-lg-12 stretch-card">
 		<div class="card">
 			<div class="card-body">
 				<h4 class="card-title">List Exams
								...
								if ($res != '') {
									...
									while ($data = $res->fetch_assoc()) {
										...
										if ($db->permission('update_exam')) {
											if ($ACTIVE == 1)
												$ACTIVE = '<a href="javascript:void(0)" style="color:#3c763d" onclick="changeExamStatus(' . $EXAM_ID . ',0)"><i class="mdi mdi-check"></i>Active</a>';
											elseif ($ACTIVE == 0)
												$ACTIVE = '<a href="javascript:void(0)" style="color:#f00" onclick="changeExamStatus(' . $EXAM_ID . ',1)"><i class="fa fa-times"></i>In-Active</a> ';

											$SHOW_RESULT 	= ($SHOW_RESULT == 1) ? '<a href="javascript:void(0)" onclick="changeExamDispResultFlag(' . $EXAM_ID . ',0)"><i class="mdi mdi-check"></i></a>' : '<a href="javascript:void(0)" onclick="changeExamDispResultFlag(' . $EXAM_ID . ',1)"><i class="mdi mdi-close"></i></a>';

											$DEMO_TEST 		= ($DEMO_TEST == 1) ? '<a href="javascript:void(0)" onclick="changeExamDemoFlag(' . $EXAM_ID . ',0)"><i class="mdi mdi-check"></i></a>' : '<a href="javascript:void(0)" onclick="changeExamDemoFlag(' . $EXAM_ID . ',1)"><i class="mdi mdi-close"></i></a>';
										} else {
											if ($ACTIVE == 1)
										...
										if ($db->permission('update_exam'))
											$action .= "<a href='page.php?page=updateExam&id=$EXAM_ID' class='btn btn-primary table-btn ' title='Edit'><i class='mdi mdi-grease-pencil'></i></a>";
										if ($db->permission('delete_exam'))
											$action .= "<a href='javascript:void(0)' onclick='deleteExam($EXAM_ID)' class='btn btn-danger table-btn ' title='Delete'><i class='mdi mdi-delete'></i></a>
									";

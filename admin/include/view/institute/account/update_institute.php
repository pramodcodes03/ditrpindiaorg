...
$user_role_result = $institute->get_role($institute_id);

if ($user_role_result) {
	while ($data = $user_role_result->fetch_assoc()) {
		$user_role = $data['USER_ROLE']; // Correctly access USER_ROLE
	}
}

/* get institute details */
$res = $institute->list_institute($institute_id, NULL, $user_role);
if ($res != '') {
	$srno = 1;

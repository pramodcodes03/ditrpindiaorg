				...
				<div class="table-responsive pt-3">
					<table id="order-listing" class="table">
						...
						<tbody>
							...
							$account = new account();
							$res = $account->list_admin_staff('', $_SESSION['user_id']);
							if ($res != '') {
								$srno = 1;
								while ($data = $res->fetch_assoc()) {
									...
									if ($db->permission('update_staff'))
										$editLink .= "<a href='page.php?page=updateStaff&id=$STAFF_ID' class='btn btn-primary table-btn' title='Edit'><i class=' mdi mdi-grease-pencil'></i></a>";

									$deleteLink = "";
									if ($db->permission('delete_staff'))
										$deleteLink .= "<a href='javascript:void(0)' onclick='deleteStaff($STAFF_ID,$USER_LOGIN_ID)' class='btn btn-danger table-btn ' title='Delete'><i class=' mdi mdi-delete'></i></a>";

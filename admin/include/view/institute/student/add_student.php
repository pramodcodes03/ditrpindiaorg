																	...
																	<tr id="courserow-<?= $i ?>">
																		...
																		<td>
																			<select class="form-control" name="course[]" id="course<?= $i ?>" onchange="getInstCourseFees(this.value, this.id)">
																				<?php
																				$sql = "SELECT A.INSTITUTE_COURSE_ID, A.COURSE_ID,A.MULTI_SUB_COURSE_ID, A.COURSE_TYPE,A.COURSE_FEES FROM institute_courses A WHERE A.INSTITUTE_ID='$institute_id' AND A.DELETE_FLAG=0";
																				$ex = $db->execQuery($sql);
																				if ($ex && $ex->num_rows > 0) {

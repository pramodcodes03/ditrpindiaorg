...
<div class="content-wrapper">
	<div class="col-lg-12 stretch-card">
		<div class="card">
			<div class="card-body">
				<h4 class="card-title">View Question Banks Details
				...
				<div class="box-header">
					Course Name: <select class="form-control select2" onchange="changeCourseQueBank(this.value)">
						<?php
						echo $db->MenuItemsDropdown("exam_question_bank A", "COURSE_ID", "COURSE_NAME", "DISTINCT A.COURSE_ID, 	get_course_title_modify(A.COURSE_ID) AS COURSE_NAME", $course_id, " ");
						?>
					</select>
				</div>
				<div class="table-responsive pt-3">
					<table id="order-listing" class="table">
						...
						<tbody>
							<?php
							$res = $exam->view_quetion_bank('', " AND COURSE_ID='$course_id'");
							$html = '';
							if ($res != '') {
								$srno = 1;
								while ($data = $res->fetch_assoc()) {
									...
									if ($db->permission('update_question'))
										$action .= "<a href='page.php?page=editQuestion&id=$QUESTION_ID' class='btn btn-primary table-btn' title='Edit'><i class='mdi mdi-grease-pencil'></i></a>";

									if ($db->permission('delete_question'))
										$action .= "<a href='javascript:void(0)' onclick='deleteQuestion($QUESTION_ID)' class='btn btn-danger table-btn' title='Delete'><i class='mdi mdi-delete'></i></a>";

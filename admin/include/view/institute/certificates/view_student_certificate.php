...
if ($checkstud != '' && !empty($checkstud)) {
	...
	if ($resB != '') {
		...
		while ($dataB = $resB->fetch_assoc()) {
			...
			$certificate_image    = $_SERVER['DOCUMENT_ROOT'] . '/uploads/background_images/' . $imageId . '/' . $certificate_image;
		}
	}
	...
	$PHOTO = $_SERVER['DOCUMENT_ROOT'] . '/uploads/default_user.png';

	if ($STUDENT_PHOTO != '')
		$PHOTO = $_SERVER['DOCUMENT_ROOT'] . '/uploads/student/' . $STUDENT_ID . '/' . $STUDENT_PHOTO;
	//==============================================================
	$STUD_SIGN = $_SERVER['DOCUMENT_ROOT'] . '/uploads/student/' . $STUDENT_ID . '/' . $STUDENT_SIGN;
	...
	$INSTITUTE_SIGN = $_SERVER['DOCUMENT_ROOT'] . '/uploads/institute/docs/' . $INSTITUTE_ID . '/' . $sign_photo;

	$INSTITUTE_STAMP = $_SERVER['DOCUMENT_ROOT'] . '/uploads/institute/docs/' . $INSTITUTE_ID . '/' . $stamp_photo;

	$file1 = "";
	if ($QRFILE !== '') {
		// $file1 =  HTTP_HOST_ADMIN . '/' . $QRFILE;
    $file1 = $_SERVER['DOCUMENT_ROOT'] . '/admin/' . $QRFILE;
	...
	// Build image HTML only if files exist to prevent mPDF errors
	$photoImg = '';
	if ($PHOTO != '' && file_exists($PHOTO)) {
		$photoImg = '<img src="' . $PHOTO . '" style="width:80px;height:100px;" />';
	}
	$studSignImg = '';
	if ($STUD_SIGN != '' && file_exists($STUD_SIGN)) {
		$studSignImg = '<img src="' . $STUD_SIGN . '" style="width:100px;height:35px;" />';
	}
	$instSignImg = '';
	if ($INSTITUTE_SIGN != '' && file_exists($INSTITUTE_SIGN)) {
		$instSignImg = '<img src="' . $INSTITUTE_SIGN . '" style="width:135px;height:40px;" />';
	}
	$qrImg = '';
	if ($file1 != '' && file_exists($file1)) {
		$qrImg = '<img src="' . $file1 . '" style="width:65px;height:65px;" />';
	}
	$bgImg = '';
	if (isset($certificate_image) && $certificate_image != '' && file_exists($certificate_image)) {
		$bgImg = '<img src="' . $certificate_image . '" style="width:100%" />';
	}
...
<div class="qrcodeimage">' . $qrImg . '</div>
	<div class="studphoto">' . $photoImg . '</div>
	<div class="studsign">' . $studSignImg . '</div>
				...
				<div class="instsign">' . $instSignImg . '</div>
				<div class="line">---------------------------------------------</div>

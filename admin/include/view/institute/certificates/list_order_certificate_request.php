 ...
 <div class="content-wrapper">
 	<div class="col-lg-12 stretch-card">
 		<div class="card">
 			<div class="card-body">
 				<h4 class="card-title">All Order Certificate Requests
 				</h4>
 				<?php
 				...
 				<div class="row">
 					<div class="col-md-12">
 						<form action="printOrderCertificate" method="post" onsubmit="return confirm('Confirm! Print certificates?'); pageLoaderOverlay('show')">
 							...
 							<div class="box box-warning">
 								...
 								<div class="box-body">
 									<div class="table-responsive">
 										<?php
											...
											$pageUrl = 'listOrderRequestedCertificates';
											/* Pagination COde */
                    			 ...
                    			 FROM certificate_order_requests_master A LEFT JOIN user_login_master B ON A.INSTITUTE_ID=B.USER_ID LEFT JOIN postal_dispatch C ON 
                    			 A.CERTIFICATE_REQUEST_MASTER_ID = C.CERTIFICATE_REQUEST_MASTER_ID WHERE A.DELETE_FLAG=0  AND B.USER_ROLE=8 $cond ORDER BY A.CREATED_ON DESC LIMIT $offset, $rec_limit";
                    					...
                    					*/
														...
														$printCert = "<a href='page.php?page=printFranchiseAddress&inst[]=$INSTITUTE_ID' class='btn btn-primary' title='Print Address' target='_blank'><i class='  mdi mdi-message-text'></i></a>";
														$inst_mobile = $db->get_user_mobile($INSTITUTE_ID, 2);
														$action1 .= "<a href='javascript:void(0)' class='btn btn-primary send-cert-dispatch-sms send-parcel-details' title='Send Dispatch SMS' data-toggle='modal' data-target='.cert_disptach_sms' data-total='$TOTAL_REQ' data-id='$CERTIFICATE_REQUEST_MASTER_ID' data-name='$INSTITUTE_NAME' data-mobile='$inst_mobile' data-instid='$INSTITUTE_ID' data-receipt='$RECEIPTNO' data-smsmessage='$PREVIEW_SMS' data-status='$DISPATCH_STATUS' data-dispatchid='$DISPATCH_ID'><i class='  mdi mdi-message-text'></i></a>";
														...
														$print_color = "";
														if ($PRINT_CERT == 1) {
															$print_color = "style='background-color:#d6ffd6;'";
															$PRINT_CERT = '<a class="btn btn-success btn-xs" href="javascript:void(0)" style="color:#FFF" onclick="changePrintStatus(' . $CERTIFICATE_REQUEST_MASTER_ID . ',0)"><i class="mdi mdi-check"></i> YES PRINTED</a>';
														} elseif ($PRINT_CERT == 0) {
															$PRINT_CERT = '<a class="btn btn-danger btn-xs" href="javascript:void(0)" style="color:#FFF" onclick="changePrintStatus(' . $CERTIFICATE_REQUEST_MASTER_ID . ',1)"><i class="mdi mdi-close"></i> NO PRINT</a>';
														}
														...
														$srno = 1;
														$maintbl .= '
                    								<tr id="req-' . $CERTIFICATE_REQUEST_MASTER_ID . '" ' . $print_color . '>
                    							...
                    							<!-- <th>Certificate Status</th> -->		
                    								<th>Date</th>
															...
															while ($data = $res->fetch_assoc()) {
																...
																$condRequest = '';
																if ($COURSE_ID != '') {
																	$condRequest = " AND COURSE_ID = '$COURSE_ID'";
																}
																if ($MULTI_SUB_COURSE_ID != '') {
																	$condRequest = " AND MULTI_SUB_COURSE_ID = '$MULTI_SUB_COURSE_ID'";
																}
																if ($TYPING_COURSE_ID != '') {
																	$condRequest = " AND TYPING_COURSE_ID = '$TYPING_COURSE_ID'";
																}
																$request_id = $db->get_certificate_request_id("STUDENT_ID = '$STUDENT_ID' AND INSTITUTE_ID = '$INSTITUTE_ID' $condRequest ");
                    								...
                    								$PHOTO = STUDENT_DOCUMENTS_PATH.'/'.$STUDENT_ID.'/'.$STUDENT_PHOTO;*/
																...
																if ($check_print != '') {
																	...
																	if ($db->permission('view_certificate'))
																		$action .= "<a href='page.php?page=certificatePrint&checkstud=$STUDENT_ID&certreq=$request_id&course=$COURSE_ID&course_multi_sub=$MULTI_SUB_COURSE_ID&course_typing=$TYPING_COURSE_ID' target='_blank' class='btn btn-primary table-btn' title='View Certificate'><i class='mdi mdi-eye'></i></a>";
																	$action .= "<a href='page.php?page=marksheetPrint&checkstud=$STUDENT_ID&certreq=$request_id&course=$COURSE_ID&course_multi_sub=$MULTI_SUB_COURSE_ID&course_typing=$TYPING_COURSE_ID' target='_blank' class='btn btn-primary table-btn' title='View Marksheet'><i class='mdi mdi-file-pdf'></i></a>";


																	//$action .= "<a href='print-order-modify-certificate&cert_detail_id=$CERTIFICATE_DETAILS_ID' target='_blank' class='btn btn-primary table-btn' title='Update Certificate'><i class='mdi mdi-grease-pencil'></i></a>";

																} else {
																	if ($db->permission('print_certificate'))
																		$action .= "<a href='page.php?page=printOrderCertificate&checkstud[]=$CERTIFICATE_REQUEST_ID' class='btn btn-primary btn1' title='Print'><i class='mdi mdi-download'></i>Print</a>";
																}
 	...
 	<div class="modal-dialog" role="document">
 		<div class="modal-content">
 			...
 			<div class="modal-header">
 				<h3 class="modal-title">Send Disptach Detail SMS</h3>
 			...
 			<form id="send_cert_dispatch_sms_form" method="post" class="form-horizontal">
 				...
 				<div class="modal-body">
 					...
 					<div class="form-group row" id="msg-error">

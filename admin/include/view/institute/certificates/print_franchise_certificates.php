...
if ($inst != '' && !empty($inst)) {
	...
	foreach ($inst as $id) {
		...
		// Check if QR file exists and prepare it for display
		if (!empty($QRFILE)) {
			// For PDF display, use full server path
			if (file_exists($QRFILE)) {
				$file1 = $QRFILE; // Use local path for mPDF
			} else {
				// Fallback to HTTP path if local file doesn't exist
				$file1 = HTTP_HOST . $QRFILE;
			}

			// Generate QR image HTML
			$qr_image_html = '<div class="qrcodeimage"><img src="' . $file1 . '"></div>';
		} else {
			// If QR doesn't exist, show placeholder or generate it
			// You can add QR generation code here if needed
			$qr_image_html = '<div class="qrcodeimage" style="border: 1px dashed #ccc; padding: 10px; text-align: center; font-size: 8px;">QR Code<br>Not Generated</div>';
		}
...
<style>
    ...
    .qrcodeimage{position:absolute;top:35.1%;width:85px; height:85px; left:80%;}
		...
		// Build HTML content with QR code
		$html = '	' . $qr_image_html . ' 

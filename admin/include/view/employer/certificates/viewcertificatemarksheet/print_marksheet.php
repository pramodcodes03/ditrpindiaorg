...
if($course_multi_sub!=='' && !empty($course_multi_sub)){
   ...
   if($COURSE_DURATION!='')
	...
	.courseperiod{position:absolute;top:30.2%;left:73.4%; width:40%; font-size:12px; font-weight:normal;}
    ...
    table tr.highlight td{padding-top:1%; padding-bottom:1%; font-size:12px;}
    ...
    .qrheadtext{position:absolute;top:78.3%;left:54.5%; font-size:10px;font-weight:900; text-align:center; width:170px;}
    .qrcodeimage{position:absolute;top:82%;width:70px; height:70px; text-align:center; float:right; left:60%;}
	</style>';
	...
	if($MULTI_SUB_COURSE_ID!='' && !empty($MULTI_SUB_COURSE_ID) && $MULTI_SUB_COURSE_ID!=0){
		...
		$marksHtml='<div style="position:absolute; top:485px; right:115px; left:135px;">';
		$res2 = $exammultisub->list_student_exam_results_multi_sub_list('',$STUDENT_ID,$INSTITUTE_ID,$STUD_COURSE_ID,'');
					...
					while($data2 = $res2->fetch_assoc())
					{
						...
						$marksHtml .="<td width='25%'>$srno1)  $SUBJECT_NAME1 <br/> &nbsp;&nbsp;&nbsp;&nbsp;<span class='subdetails'>$subdetails </span></td>";
						$marksHtml .="<td width='5%'>$MARKS_OBTAINED1</td>";
						$marksHtml .="<td width='5%'>$PRACTICAL_MARKS1</td>";	
						$marksHtml .="<td width='5%'>$totalmks</td>";	
						$marksHtml .= '</tr>';
	...
	if($QRFILE !== '' && !empty($QRFILE)){
	$html2 .='<h3 class="qrheadtext"><b> SCAN <br/> FOR ONLINE VERIFICATION </b></h3>
   <div class="qrcodeimage"><img src="'.$files.'"></div>';
	}

	if($ISSUE_DATE >= '2023-11-09'){
			$backgroundImage = "resources/dist/img/newMarksheet.jpeg";  
	}else{
		$backgroundImage = "resources/dist/img/marksheet.JPG";
	}
   
	$html2 .= '<img src="'.$backgroundImage.'" style="width:100%" />';
	$html2 .= $html;

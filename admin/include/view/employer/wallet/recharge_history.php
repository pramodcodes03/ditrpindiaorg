...
<div class="content-wrapper">
	<div class="row">
		<div class="col-12 grid-margin stretch-card">
			<div class="card">
				<div class="card-body">
					<h4 class="card-title">Wallet Recharge History</h4>
							...
							<form action="" method="post" onsubmit="pageLoaderOverlay('show')">
								...
								<div class="form-group col-sm-2">
									...
									<select class="form-control" name="user_role" onchange="getUserListByRole(this.value)">
										<?php
										echo $db->MenuItemsDropdown('user_role_master', 'USER_ROLE_ID', 'STATUS_NAME', 'USER_ROLE_ID,STATUS_NAME', $user_role, " WHERE USER_ROLE_ID IN(4)");
										?>
									</select>
								</div>
								...
								<div class="form-group col-sm-4">
									...
									<select class="form-control select2" name="user_id" id="userlist" onchange="getUserDetails(this.value)">
										<?php
										if ($user_role == 2)
											echo $db->MenuItemsDropdown('institute_details', 'INSTITUTE_ID', 'INSTITUTE_NAME', 'INSTITUTE_ID,CONCAT(INSTITUTE_NAME," - ",INSTITUTE_CODE) AS INSTITUTE_NAME', $institute_id, " WHERE DELETE_FLAG=0 AND ACTIVE=1");
										else if ($user_role == 3)
											echo $db->MenuItemsDropdown('employer_details', 'EMPLOYER_ID', 'EMPLOYER_COMPANY_NAME', 'EMPLOYER_ID,EMPLOYER_COMPANY_NAME', $institute_id, " WHERE DELETE_FLAG=0 AND ACTIVE=1");
										else if ($user_role == 4)
											echo $db->MenuItemsDropdown('student_details', 'STUDENT_ID', 'get_student_name(STUDENT_ID)', 'STUDENT_ID,get_student_name(STUDENT_ID)', $institute_id, " WHERE DELETE_FLAG=0 AND ACTIVE=1");
										?>
									</select>
								</div>
								<div class="form-group col-sm-2">
								...
								<div class="form-group col-sm-1">
									<label> &nbsp;</label>
									<a class="form-control btn btn-sm btn-warning" onclick="pageLoaderOverlay('show'); location.assign('rechargeHistory')">Clear</a>
								</div>
								<!--
					<div class="form-group col-sm-1">
					<label> &nbsp;</label>				 
...
*/
									$BONUS_STAUS = '';
									$history = $admin->get_recharge_history($paymentmode1, $wallet_id, $institute_id, $user_role, $cond);
									arsort($history);
									//print_r($history);
									...
									if (!empty($history)) {
										...
										foreach ($history as $trans => $transArr) {
											if (is_array($transArr) && !empty($transArr)) {
												...
												if ($USER_ROLE == '4') $USER_ROLE = 'Student';
												if ($BONUS_STAUS == '1') {
													$BONUS_STAUS = 'YES';
												}
												if ($BONUS_STAUS == '0') {

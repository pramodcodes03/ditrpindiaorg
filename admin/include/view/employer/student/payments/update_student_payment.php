...
if ($payments != '') {
	...
	while ($courseData = $payments->fetch_assoc()) {
		...
		$feesdetails = $institute->get_stud_course_fee_detail($STUDENT_ID, $INSTITUTE_COURSE_ID, '');
					...
					<form class="forms-sample" action="" method="post" enctype="multipart/form-data">
						...
						<div class="row">
							...
							<div class="col-md-6 form-group  <?= (isset($errors['amountpaid'])) ? 'has-error' : '' ?>">
								...
								<input type="hidden" id="feespaid1" value="<?= $FEES_PAID ?>" />
								<input type="text" name="amountpaid" id="amountpaid" class="form-control" value="<?= isset($_POST['amountpaid']) ? $_POST['amountpaid'] : $FEES_PAID; ?>" onchange="addPayShowBal1()" onkeyup="addPayShowBal1()" />

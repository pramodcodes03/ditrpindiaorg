		...
		<div class="col-12 grid-margin stretch-card">
			<div class="card">
				<div class="card-body">
					...
					<div class="col-sm-9">
						<textarea class="form-control" id="instdetails" name="instdetails" placeholder="Please provide details about Staff,Infrastructure, Current business, and Reason of joining DITRP" type="text"><?= isset($_POST['instdetails']) ? $_POST['instdetails'] : $DETAIL_DESCRIPTION ?></textarea>
								...
								<div class="form-group col-sm-4 <?= (isset($errors['pword'])) ? 'has-error' : '' ?>">
									...
									<input class="form-control" id="pword" name="pword" placeholder="New Password" value="<?= isset($_POST['pword']) ? $_POST['pword'] : '' ?>" type="password" autocomplete="off">
					...
					<div class="form-group">
					<?php
					$prime = isset($_POST['prime']) ? $_POST['prime'] : $PRIMEMEMBER;
					?>
					<label for="prime" class="col-sm-3 control-label">Prime Member</label>
					<div class="radio">
						<label>
						...
						No
								...
								<div class="col-md-12">
									...
									<div class="box box-primary">
										...
										<div class="box-body row">
											...
											<!--<div class="form-group <?= (isset($errors['inststamp'])) ? 'has-error' : '' ?>">
									<label for="inststamp" class="col-sm-4 control-label">Institute Stamp</label>
									<div class="col-sm-8">
									<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'stamp', true); ?>
									<input id="inststamp" name="inststamp" type="file">
									<p class="help-block"><?= (isset($errors['inststamp'])) ? $errors['inststamp'] : 'Stamp' ?></p>
									</div>
								</div> -->

											<div class="form-group col-sm-4 <?= (isset($errors['photoidproof'])) ? 'has-error' : '' ?>">
												<label>Photo ID Proof</label>
												<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'owner_photo_id_proof', true);	?>
												<input id="photoidproof" name="photoidproof" type="file">
												<p class="help-block"><?= (isset($errors['photoidproof'])) ? $errors['photoidproof'] : 'Adhaar Card, Voter Id, Driving Licence, Bank Passbook with Photo, etc.' ?></p>
											</div>

											<div class="form-group col-sm-4 <?= (isset($errors['instregcertificate'])) ? 'has-error' : '' ?>">
												<label>Institute Registration Certificate</label>
												<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'institute_registration_certificate', true);	?>
												<input id="instregcertificate" name="instregcertificate" type="file">
												<p class="help-block"><?= (isset($errors['instregcertificate'])) ? $errors['instregcertificate'] : 'ShopAct, Grampanchayat NOC, Company/Society Registration' ?></p>
											</div>

											<div class="form-group col-sm-4 <?= (isset($errors['educationalproof'])) ? 'has-error' : '' ?>">
												<label>Educational Qualification </label>
												<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'educational_certificates', true);	?>
												<input id="educationalproof" name="educationalproof" type="file">
												<p class="help-block"><?= (isset($errors['educationalproof'])) ? $errors['educationalproof'] : '10th, 12th, Graduation, Post Graduation, Doctorate, Other ' ?></p>
											</div>

											<div class="form-group col-sm-4 <?= (isset($errors['profcourseproof'])) ? 'has-error' : '' ?>">
												<label>Professional Courses Completed </label>
												<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'professional_courses_certificates', true);	?>
												<input id="profcourseproof" name="profcourseproof" type="file">
												<p class="help-block"><?= (isset($errors['profcourseproof'])) ? $errors['profcourseproof'] : 'Any other special courses, other than university/formal education' ?></p>
											</div>

											<div class="form-group col-sm-4 <?= (isset($errors['instphotos'])) ? 'has-error' : '' ?>">
												<label>Photo(s) of Franchise</label>
												<?= $institute->get_institute_docs_all($INSTITUTE_ID, 'other_photos', true);	?>
												<input id="instphotos" name="instphotos[]" multiple type="file">
												<p class="help-block"><?= (isset($errors['instphotos'])) ? $errors['instphotos'] : 'Franchise Photos' ?> </p>
											</div>
										</div>
									</div>
								</div>
								...
								<div class="form-group <?= (isset($errors['democount'])) ? 'has-error' : '' ?>">
									...
									<div class="col-sm-9">
										...
										<span class="help-block"><?= (isset($errors['democount'])) ? $errors['democount'] : '' ?></span>
											...
											<?php if ($VERIFIED == 1) { ?>
												<div class="form-group col-sm-4">
													<label for="verifydate" class="col-sm-3 control-label">Verified On</label>
								...
								<div class="col-md-12">
									<div class="box box-primary">
										...
										<div class="box-body row">
											<div class="form-group col-sm-4 <?= (isset($errors['masterpassword'])) ? 'has-error' : '' ?>">
												<input class="form-control pull-right" value="<?= isset($_POST['masterpassword']) ? $_POST['masterpassword'] : ''; ?>" id="masterpassword" type="text" name="masterpassword">
												<span class="help-block"><?= (isset($errors['masterpassword'])) ? $errors['masterpassword'] : '' ?></span>
											</div>
										</div>
									</div>
								</div>

          ...
          <form class="forms-sample" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <select class="form-control form-control-sm" id="exampleFormControlSelect3" id="courseid" name="courseid" onchange="getSubjectId(this.value)">
                <?php
                $courseid = isset($_POST['courseid']) ? $_POST['courseid'] : $AICPE_COURSE_ID;
                echo $db->MenuItemsDropdown('courses A LEFT JOIN exam_structure B ON A.COURSE_ID=B.AICPE_COURSE_ID', 'COURSE_ID', 'COURSE', 'A.COURSE_ID, get_course_title_modify(A.COURSE_ID) AS COURSE', $courseid, ' WHERE B.AICPE_COURSE_ID IS NULL AND A.ACTIVE=1 AND A.DELETE_FLAG=0 ORDER BY A.COURSE_CODE ASC');
                ?>
              </select>
            </div>
            ...
            <div class="form-group">
              <label for="exampleInputName1">Total Marks</label>
              <input type="number" class="form-control" id="exampleInputName1" name="totalmarks" placeholder="totalmarks" value="<?= isset($_POST['totalmarks']) ? $_POST['totalmarks'] : $TOTAL_MARKS ?>" onchange="setMarkPerQue()">
            </div>
            <div class="form-group">
              <label for="exampleInputName1">Total Questions</label>
              <input type="number" class="form-control" id="exampleInputName1" name="totalque" placeholder="totalque" value="<?= isset($_POST['totalque']) ? $_POST['totalque'] : $TOTAL_QUESTIONS ?>" onchange="setMarkPerQue()">
            </div>
            <div class="form-group">
              <label for="exampleInputName1">Marks Per Questions</label>
              <input type="text" class="form-control" id="exampleInputName1" name="markperque" placeholder="markperque" value="<?= isset($_POST['markperque']) ? $_POST['markperque'] : $MARKS_PER_QUE ?>" readonly>
            </div>

            <div class="form-group">
              <label for="exampleInputName1">Passing Marks( % )</label>
              <input type="text" class="form-control" id="exampleInputName1" name="passingmarks" placeholder="passingmarks" value="<?= isset($_POST['passingmarks']) ? $_POST['passingmarks'] : $PASSING_MARKS ?>">
            </div>

            <div class="form-group">
              <label for="exampleInputName1">Exam Time (minutes)</label>
              <input type="number" class="form-control" id="exampleInputName1" name="examtime" placeholder="examtime" value="<?= isset($_POST['examtime']) ? $_POST['examtime'] : $EXAM_TIME ?>">
            </div>

            <div class="form-group">
              <label for="exampleInputName1">Exam Modes</label>
              <?php
              ...
              ?>
                  <label>
                    <input name="exam_mode[]" value="<?= $EXAM_TYPE_ID ?>" <?= (in_array($EXAM_TYPE_ID, $exam_mode)) ? "checked=''" : ''  ?> type="checkbox">
                    <?= $EXAM_TYPE ?>
                  </label>
              <?php
                }

	...
	if($res2!='')
{
	while($data2 = $res2->fetch_assoc())
	{
		...
		$STUD_PHOTO = STUDENT_DOCUMENTS_PATH.'/'.$STUDENT_ID.'/'.$studentData['STUDENT_PHOTO'];
		 $STUD_SIGN = STUDENT_DOCUMENTS_PATH.'/'.$STUDENT_ID.'/'.$studentData['STUDENT_SIGN'];
    ...
    .studsign{position:absolute;  width:180px; height:45px;background-image:url("'.$STUD_SIGN.'"); background-size:180px 45px; background-repeat:no-repeat; top:72.2%; left:60%;}
	...
	.institutecontactnumber{position:absolute; top:62.5%; left:77%; font-size:9pt;}
		...
		$html .="<div class='studsign'></div>";
		
		$html .="<div class='studusername'><b>".$USER_NAME."</b></div>";
		$html .="<div class='studpassword'><b>".$PASS_TEXT."</b></div>";

...
if($course_multi_sub!=='' && !empty($course_multi_sub)){
	...
	$res = $access->list_printed_certificates('','', $cond);	
	...
	.studname{position:absolute;top:26%;left:29.7%;width:35%}
	.fathername{position:absolute;top:27.6%;left:29.7%; width:35%}
	.surname{position:absolute;top:29%;left:29.7%; width:35%}
	.institudename{position:absolute;top:35.4%;left:29.7%;width:100%}
	
	.dob{position:absolute;top:29.3%;left:72.5%; width:15%}
	
	.courseperiod{position:absolute;top:30.7%;left:72.5%; width:40%; font-size:12px; font-weight:normal;}
	
	.coursedur{position:absolute;top:25.8%;left:72.5%; width:15%}
	
	.coursename{position:absolute;top:32.3%;left:29.7%; border:1px }
	
	.mothername{position:absolute;top:30.4%;left:29.7%; width:23%}
	
	.marksheetno{position:absolute;top:27.5%;left:72.5%; width:25%; font-size:10px;}
	
	.subject{position:absolute;top:42.2%;left:17%; border:1px;width:42%; text-transform: uppercase; text-align:justify; line-height:20px;}
	.subjectmark{position:absolute;top:44.2%;left:72.6%; height:50px;width:50px;font-size:14px; }
	.total{position:absolute;top:44.2%;left:79.7%; height:50px;width:50px;font-size:14px; }
	.practicle{position:absolute;top:44.2%;left:64.6%; height:50px;width:50px;font-size:14px; }
	.total_bottom{position:absolute;top:69.5%;left:79%; font-size:16px;height:50px;width:50px; }
    ...
    .period_date{font-weight:normal;font-size: 8pt; }
     ...
     .tagmark{position:absolute;bottom:6%;left:40%; font-size:18px;color:#000;}
	...
	if($AICPE_COURSE_ID!='' && !empty($AICPE_COURSE_ID) && $AICPE_COURSE_ID!=0){
		...
		$html .="<div class='courseperiod'><span class='period_date'>$start_course_date To $end_course_date </span></div>";
		...
		$html .="<div class='tagmark'> www.ditrppro.com </div>";
	}
	if($MULTI_SUB_COURSE_ID!='' && !empty($MULTI_SUB_COURSE_ID) && $MULTI_SUB_COURSE_ID!=0){
		...
		$marksHtml='<div style="position:absolute; top:485px; right:110px; left:145px;">';
		$res2 = $exammultisub->list_student_exam_results_multi_sub_list('',$STUDENT_ID,$INSTITUTE_ID,$STUD_COURSE_ID,'');
					...
					while($data2 = $res2->fetch_assoc())
					{
						...
						$marksHtml .="<td width='25%'>$srno1)  $SUBJECT_NAME1</td>";
						$marksHtml .="<td width='4%' style='margin-left:8px; left:10px;'>$MARKS_OBTAINED1</td>";
						$marksHtml .="<td width='4%' style='margin-left:8px; left:10px;'>$PRACTICAL_MARKS1</td>";	
						$marksHtml .="<td width='4%' style='margin-left:8px; left:10px;'>$totalmks</td>";	
						$marksHtml .= '</tr>';

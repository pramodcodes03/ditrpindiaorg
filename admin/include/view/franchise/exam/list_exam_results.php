		...
		<div class="card">
			<div class="card-body">
				...
				<form action="" method="post" class="form-inline"
					onsubmit="return confirmApplyForCertificate(this);">
							...
							<tbody>
								...
								if ($exam_result_info_multi_sub != '') {
									...
									while ($data = $exam_result_info_multi_sub->fetch_assoc()) {
										...
										$action1 .= '<a href="page.php?page=updatePracticalExamResult&result=&result_multi_sub=' . $EXAM_RESULT_FINAL_ID . '&result_typing=" class="btn btn-xs btn-primary">Marksheet</a>';
...
<script>
	function confirmApplyForCertificate(form) {
		if (confirm('Confirm! Apply for certificates ?')) {
			let btn = form.querySelector('input[type="submit"][name="submit"]');
			btn.disabled = true;
			btn.value = "Processing..."; // Optional: Change button text
			pageLoaderOverlay('show'); // keep your loader
			return true; // allow form submit
		}
		return false; // cancel submit if confirm = false
	}
</script>
 <?php
  ...
  //$conn = mysqli_connect("localhost","root","","excel_import");
      ...
      for ($i = 0; $i < $sheetCount; $i++) {
        ...
        foreach ($Reader as $Row) {
          if ($S > $C) {
            ...
            $fname                      = strtoupper($db->test(isset($Row[3]) ? $Row[3] : ''));
            $mname                      = strtoupper($db->test(isset($Row[4]) ? $Row[4] : ''));
            $lname                      = strtoupper($db->test(isset($Row[5]) ? $Row[5] : ''));
            $mothername                 = strtoupper($db->test(isset($Row[6]) ? $Row[6] : ''));

            $mobile                     = $db->test(isset($Row[7]) ? $Row[7] : '');
            $dob                        = $db->test(isset($Row[8]) ? $Row[8] : '');
            $gender                     = $db->test(isset($Row[9]) ? $Row[9] : '');

            $adharid                    = isset($Row[10]) ? $Row[10] : '';

            $postcode                   = $db->test(isset($Row[11]) ? $Row[11] : '');
            $per_add                    = $db->test(isset($Row[12]) ? $Row[12] : '');
            ...
            $tabFields  = "(ENQUIRY_ID,INSTITUTE_ID, ABBREVIATION, STUDENT_FNAME,STUDENT_MNAME,STUDENT_LNAME,STUDENT_MOTHERNAME, STUDENT_DOB,STUDENT_GENDER,STUDENT_MOBILE,STUDENT_PER_ADD,STUDENT_PINCODE,STUDENT_ADHAR_NUMBER,INSTRESTED_COURSE,CERT_MNAME,CERT_LNAME,ENQUIRY_BY, CREATED_BY, CREATED_ON, CREATED_ON_IP)";

            $insertVals = "(NULL, '$institute_id','$abbreviation','$fname','$mname','$lname','$mothername','$dob','$gender','$mobile','$per_add','$postcode','$adharid','[$interested_course]','1', '1','$institute_id','$created_by',NOW(),'$created_by_ip')";

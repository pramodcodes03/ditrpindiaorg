...
if($id!='')
{
	...
	if($TYPING_COURSE_ID!='' && !empty($TYPING_COURSE_ID) && $TYPING_COURSE_ID!='0'){
		$COURSE_NAME = $student->get_student_coursesname_typing($TYPING_COURSE_ID);		
		$COURSE_CODE = $student->get_student_coursescode_typing($TYPING_COURSE_ID);
	}
	...
	while($data4 = $payments->fetch_assoc()){
		{
			...
			$ADMISSION_DATE = date("d-m-Y",strtotime($data4['ADMISSION_DATE']));
...
<style>
	...
	.rollnumber{position:absolute;top:275px;text-align:left;width:100%; font-size:12px; left:53%;}
	...
	$html = '	
           ...
           if($TYPING_COURSE_ID!='' && !empty($TYPING_COURSE_ID) && $TYPING_COURSE_ID!='0'){
              $html .= '        <p class="coursename">'.$COURSE_NAME.' ('.$COURSE_CODE.')</p>';
           }else{
          $html .= '        <p class="coursename">'.$AWARD.' IN '.$COURSE_NAME.' ('.$COURSE_CODE.')</p>';
           }
                
	     $html .= '       <p class="admissiondate">'.$ADMISSION_DATE.'</p>

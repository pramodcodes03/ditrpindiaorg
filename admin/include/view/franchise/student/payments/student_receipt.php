	...
	if($resB!='')
	{
	  ...
	  while($dataB = $resB->fetch_assoc())
	  {
		    ...
		    if($feesreceipt_image !=''){
              $feesreceipt_image    = BACKGROUND_IMAGE_PATH.'/'.$user_id.'/'.$feesreceipt_image;
              }else{
...
//	$mpdf=new mPDF('c','A5-L','','',0,0,0,0,16,13); 
    ...
    .text{position:absolute;top:390px;text-align:left;left:50px; width:100%;}
	</style>';
	$html .= '
    <img src="'.$feesreceipt_image.'" style="width:100%;" />
	    ...
	    <p class="text"> The course fee paid by the student has been paid to the above  mentioned ATC. 
	    DITRP Company is not responsible in any way for this. </p>
		';

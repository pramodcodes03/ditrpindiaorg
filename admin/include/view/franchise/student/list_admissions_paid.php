 <?php
	...
	$selsql = "SELECT *,get_student_name(A.STUDENT_ID) AS STUDENT_NAME,get_stud_photo(A.STUDENT_ID) AS STUDENT_PHOTO, DATE_FORMAT(A.CREATED_ON, '%d-%m-%Y') AS CREATED_DATE   FROM student_course_details A WHERE A.DELETE_FLAG=0 AND A.INSTITUTE_ID='$institute_id' AND A.ADMISSION_CONFIRMED=1";
 			...
 			<div class="col-xs-12">
 					...
 					<div class="box-header">
 						...
 						<?php if ($db->permission('add_enquiry')) { ?>
 							<div class="form-group col-sm-2">
 								<label> &nbsp;</label>
 								<a href="page.php?page=add-student-enquiry" class="form-control btn btn-sm btn-primary" style="height:39px"><i class="fa fa-plus"></i> Add New Admission</a>
 							</div>
 						<?php } ?>
 					</div>
 					<div class="box-body">
				...
				<div class="form-group col-sm-3">
				  ...
				  <select class="form-control select2" name="studid" id="studid">
					  <?php echo $db->MenuItemsDropdown('student_details', "STUDENT_ID", "STUDENT_NAME", "STUDENT_ID, CONCAT(CONCAT(STUDENT_FNAME,' ',STUDENT_MNAME),' ',STUDENT_LNAME) STUDENT_NAME", $studid, " WHERE INSTITUTE_ID='$institute_id' AND DELETE_FLAG=0"); ?>
					</select>	
				</div>
				...
				<div class="form-group col-sm-3">
				  <label>Courses</label>
					 ...
					 <option value="">--select--</option>
					  <?php
						$sql3 = "SELECT A.INSTITUTE_COURSE_ID, A.COURSE_ID, A.COURSE_TYPE FROM institute_courses A WHERE A.INSTITUTE_ID='$institute_id' AND A.DELETE_FLAG=0";
						$ex = $db->execQuery($sql3);
						if ($ex && $ex->num_rows > 0) {
							while ($data3 = $ex->fetch_assoc()) {
								$course 		= $db->get_course_detail($data3['COURSE_ID'], $data3['COURSE_TYPE']);
								$course_name 	= $course['COURSE_NAME_MODIFY'];
								$selected = ($data3['INSTITUTE_COURSE_ID'] == $courseid) ? 'selected="selected"' : '';
								echo '<option value="' . $data3['INSTITUTE_COURSE_ID'] . '" ' . $selected . '>' . $course_name . '</option>';
							}
						}
						?>
					 </select>
				...
				<div class="form-group col-sm-1">
				  <label> &nbsp;</label>
					<a class="form-control btn btn-sm btn-warning" onclick="pageLoaderOverlay('show'); location.assign('list-admissions')">Clear</a>
 			...
 			<div class="col-xs-12">
 				<div class="box box-warning">
					...
					*/
												...
												if ($STUDENT_PHOTO != '') {
													$STUDENT_PHOTO = STUDENT_DOCUMENTS_PATH . '/' . $STUDENT_ID . '/' . $STUDENT_PHOTO;
												} else {
													$STUDENT_PHOTO = '/default_user.png';
												}
										...
										?>
 											<tr id="row-<?= $STUD_COURSE_DETAIL_ID ?>">
 												...
 												<td class="col-md-1 col-xs-1">
 													<a href='page.php?page=view-student-form&id=<?= $STUDENT_ID ?>' class='btn btn-primary' title='View Form' target="_blank" style="width:100%">View Form</a>

 													<a href='page.php?page=view-student-idcard&id=<?= $STUDENT_ID ?>' class='btn btn-primary' title='View ID Card' target="_blank" style="width:100%">View ID Card</a>

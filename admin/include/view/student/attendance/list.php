...
$student_id         = $db->test(isset($_REQUEST['student_id']) ? $_REQUEST['student_id'] : $student_id);
...
if ($student_id != '' && $course != '') $cond .= " AND A.STUDENT_ID='$student_id' AND B.INSTITUTE_COURSE_ID = '$course'";
                ...
                <form name="form1" class="forms-sample" action="" method="post" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="form-group col-sm-3 <?= (isset($errors['batch'])) ? 'has-error' : '' ?>">
                            ...
                            ?>
                            <input type="hidden" class="form-control" placeholder="student_id" name="student_id" value="<?php echo $student_id ?>">
                            <select class="form-control select2" name="student_id" id="student_id" onchange="getStudentAllCourses(this.value); getStudPaymentInfo(); getBalAmtCourse();" readonly disabled>
                                <?php

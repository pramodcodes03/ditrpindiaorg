...
ob_clean();
	...
	if($course!=='' && !empty($course)){
    	$cond = " AND COURSE_ID='$course' AND STUDENT_ID='$studid' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
    }
    if($course_multi_sub!=='' && !empty($course_multi_sub)){
    	$cond = " AND MULTI_SUB_COURSE_ID='$course_multi_sub' AND STUDENT_ID='$studid' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
    }
	if($course_typing!=='' && !empty($course_typing)){
		$cond = " AND TYPING_COURSE_ID='$course_typing' AND STUDENT_ID='$studid' ORDER BY CERTIFICATE_DETAILS_ID DESC LIMIT 0,1";
	}
	...
	$html2 .='<div class="qrcodeimage"><img src="'.$file.'"></div>';
	 $html .="<div class='weblink'> Online Certificate Verification available on : www.ditrpindia.com </div>";

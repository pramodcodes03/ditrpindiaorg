			  ...
			  <div class="clearfix"></div> -->
 								...
 								<?php
									...
									if ($courses != '') {
										...
										while ($courseData = $courses->fetch_assoc()) {
											...
											$COURSE_TYPE 		= isset($COURSE_INFO['COURSE_TYPE']) ? $COURSE_INFO['COURSE_TYPE'] : '';

											if ($COURSE_TYPE == 1) {

												$EXAM_FEES 		= isset($COURSE_INFO['COURSE_FEES']) ? $COURSE_INFO['COURSE_FEES'] : '';

												$coursetype = 'DITRP';
											} else {

												$coursetype = 'NON-DITRP';
											}
											...
											$action = "<a href='javascript:void(0)' class='btn btn-xs btn-primary' title='Payment Info' onclick='togglePaymentInfo($STUD_COURSE_DETAIL_ID)'><i class=' fa fa-rupee'></i></a>

						

						<a href='page.php?page=list-student-courses-details&inst_course=$INSTITUTE_COURSE_ID' class='btn btn-xs btn-primary' title='Course Info'><i class='fa fa-info'></i></a>
					...
					";
										 ...
										 <td>$action</td>
											...
											$payments = $institute->list_student_payments('', $STUDENT_ID, $INSTITUTE_ID, '', ' AND A.INSTITUTE_COURSE_ID=' . $INSTITUTE_COURSE_ID);

											$paymentDetails = '<tr style="display:none;" id="paymentinfo_' . $STUD_COURSE_DETAIL_ID . '">
											...
											if ($payments != '') {
												...
												while ($paydata = $payments->fetch_assoc()) {

													$PAYMENT_ID = $paydata['PAYMENT_ID'];

													$RECIEPT_NO = $paydata['RECIEPT_NO'];

													$FEES_PAID = $paydata['FEES_PAID'];

													$TOTAL_FEES_PAID = $paydata['TOTAL_FEES_PAID'];

													$FEES_PAID_ON = $paydata['FEES_PAID_ON'];
											...
											$TOTAL_FEES_PAID = isset($TOTAL_FEES_PAID) ? $TOTAL_FEES_PAID : '';

											$paymentDetails .= '<tr class="warning" style="font-weight:bold">

														<td colspan="2">Total</td>

														<td>' . $TOTAL_FEES_PAID . '</td>
											...
											echo $courseDetail . $paymentDetails;

 <?php
	$page_id = $db->test(isset($_GET['page_id']) ? $_GET['page_id'] : '');
	$action = isset($_POST['action']) ? $_POST['action'] : '';
	if ($action != '') {

		$result = $db->update_page();
		...
		if ($success == true) {
			...
			header('location:/website_management/list-pages');
		}
	}
	$query = " SELECT * FROM page WHERE DELETE_FLAG=0 AND PAGE_ID='$page_id'";
	$exec   = $db->execQuery($query);
	$srNo = 0;
 					...
 					<div class="box-body">
 						<form role="form" action="" class="form-horizontal" name="colg_detail" method="post" enctype="multipart/form-data" onsubmit="return eventFormValidation();">
 							...
 							<div class="form-group <?= (isset($errors['page_name'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-9">
 									<input type="text" name="page_name" class="form-control" id="page_name" placeholder="Page name" value="<?= $PAGE_NAME ?>" />
 							...
 							<div class="form-group <?= (isset($errors['page_title'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-9">
 									<input type="text" name="page_title" class="form-control" id="page_title" placeholder="Page title" value="<?= $PAGE_TITLE ?>" />
 							...
 							<div class="form-group <?= (isset($errors['page_link'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-9">
 									<input type="text" name="page_link" class="form-control" id="page_link" placeholder="Page Link" value="<?= $PAGE_LINK ?>" />
 							...
 							<div class="form-group <?= (isset($errors['meta_keys'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-9">
 									<input type="text" name="meta_keys" class="form-control" id="meta_keys" placeholder="Meta keywords" value="<?= $META_TAGS ?>" />
 							...
 							<div class="form-group <?= (isset($errors['meta_desc'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-9">
 									<input type="text" name="meta_desc" class="form-control" id="meta_desc" placeholder="Meta description" value="<?= $META_DESCRIPTION ?>" />
 							...
 							<div class="form-group <?= (isset($errors['page_data'])) ? 'has-error' : '' ?>">
 								...
 								<div class="col-xs-10">
 									<textarea class="form-control" name="page_data" id="page_data" style="height:300px"><?= $PAGE_DATA ?></textarea>
 							...
 							<div class="form-group <?= (isset($errors['status'])) ? 'has-error' : '' ?>">
 								...
 								<div>
 									<label class="radio-inline">
 										<input type="radio" name="status" id="status" value="1" <?php if ($ACTIVE == 1) echo "checked"; ?> />Active
 									</label>
 									<label class="radio-inline">
 										<input type="radio" name="status" id="status2" value="0" <?php if ($ACTIVE == 0) echo "checked"; ?> />Inactive
 									</label>
 								</div>
 							...
 							<div class="form-group <?= (isset($errors['is_dynamic'])) ? 'has-error' : '' ?>">
 								...
 								<div>
 									<label class="radio-inline">
 										<input type="radio" name="is_dynamic" id="is_dynamic1" value="1" <?php if ($IS_DYNAMIC == 1) echo "checked"; ?> />Yes
 									</label>
 									<label class="radio-inline">
 										<input type="radio" name="is_dynamic" id="is_dynamic2" value="0" <?php if ($IS_DYNAMIC == 0) echo "checked"; ?> />No
 									</label>
 								</div>
 							...
 							<div class="form-group">
 								...
 								<div class="col-lg-5">
 									...
 									<a href="/website_management/list-pages" class="btn btn-danger">Cancel</a>

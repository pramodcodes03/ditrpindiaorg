...
$action = isset($_POST['edit_services']) ? $_POST['edit_services'] : '';
...
if ($action != '') {
  ...
  $result = $websiteManage->edit_services($id);
  ...
  if ($success == true) {
    ...
    header('location:/website_management/manageServices');
  }
}

$res = $websiteManage->list_services($id, '');
if ($res != '') {
  $srno = 1;
  while ($data = $res->fetch_assoc()) {
    extract($data);

    $photo = SERVICES_PATH . '/' . $id . '/' . $image;
  }
}
?>
      ...
      <div class="card">
        <div class="card-body">
          ...
          <form class="forms-sample" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              <label>Image</label>
              <input type="file" name="servicesimage" class="file-upload-default">
            ...
            <input type="submit" name="edit_services" class="btn btn-primary mr-2" value="Submit">
            <a href="/website_management/manageServices" class="btn btn-danger mr-2" title="Cancel">Cancel</a>
          </form>
        </div>
      </div>
    </div>

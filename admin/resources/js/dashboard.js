(function($) {
  ...
  $(function() {
    ...
    if ($("#south-america-chart").length) {
      ...
      $.post('include/classes/ajax.php',{action:'get_fees_details'}, function(data){	
        //console.log(data);
            var data 		= JSON.parse(data); 
            var balancefees = data[0];
            var totalfees = data[1];
            console.log(balancefees);
            console.log(totalfees);
            var per = 0;
            if(balancefees != 0 && totalfees !=0 ){
                 per = balancefees / totalfees * 100; 
            }
           

            console.log(data);
            var areaData = {
              labels: ["Balance Fees", "Total Fees"],
              datasets: [{
                  data: data,
                  backgroundColor: [
                    "#4B49AC","#FFC100"
            ...
            var areaOptions = {
              ...
              legendCallback: function(chart) { 
                var text = [];
                text.push('<div class="report-chart">');
                  text.push('<div class="d-flex justify-content-between mx-4 mx-xl-5 mt-3"><div class="d-flex align-items-center"><div class="mr-3" style="width:20px; height:20px; border-radius: 50%; background-color: ' + chart.data.datasets[0].backgroundColor[0] + '"></div><p class="mb-0">Balance Fees</p></div>');
                  text.push('<p class="mb-0">'+chart.data.datasets[0].data[0]+'</p>');
                  text.push('</div>');
                  text.push('<div class="d-flex justify-content-between mx-4 mx-xl-5 mt-3"><div class="d-flex align-items-center"><div class="mr-3" style="width:20px; height:20px; border-radius: 50%; background-color: ' + chart.data.datasets[0].backgroundColor[1] + '"></div><p class="mb-0">Total Fees</p></div>');

 ...
 *
 * Uses unbuffered MySQL query + streaming CSV output.
 * Data flows to browser row-by-row — impossible to 504 timeout.
 */
...
// ============================================================
// KILL all output buffering so data flows to nginx immediately
// ============================================================
while (ob_get_level()) {
    ob_end_clean();
}
...
// ============================================================
// Send CSV headers — browser starts download immediately
// ============================================================
...
header('Expires: 0');
header('X-Accel-Buffering: no'); // Tell nginx to disable proxy buffering
...
if (!$result) {
    fwrite($output, "ERROR: " . $mysqli->error);
    fclose($output);
    exit;
}

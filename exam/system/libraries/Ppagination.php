...
class Ppagination extends CI_Pagination {

    public $display_prev_link = false;
    public $display_next_link = false;
	public $display_first_link = false;
    public $display_last_link = false;
     ...
     * @return  string
     */
    public function create_p_linking()
    {
		...
		elseif ($this->display_first_link) {
			if($this->cur_page === 1)
			{
                $output .= $this->first_tag_open.'<span>'.$this->first_link.'</span>'.$this->first_tag_close;
			}
			else
			{
				 $first_url = ($this->first_url === '') ? $this->base_url : $this->first_url;
        ...
        if ($this->prev_link !== FALSE && $this->cur_page !== 1)
        {
            ...
            {
                $output .= $this->prev_tag_open.'<a href="'.$this->first_url.$query_string.'"'.$attributes.$this->_attr_rel('prev').' onclick="saveNnext('.$this->cur_page.'), submitFrm(2)">'
            ...
            else
            {
                ...
                $output .= $this->prev_tag_open.'<a href="'.$this->base_url.$append.'"'.$attributes.$this->_attr_rel('prev').' class="push_button gray"  onclick="saveNnext('.$this->cur_page.'), submitFrm(2)">'
                    .$this->prev_link.'</a>'.$this->prev_tag_close;
            }
...
// !!!!!!!! START EDITION !!!!!!!!!!
        elseif ($this->display_prev_link && $this->cur_page === 1) {
                ...
                $output .= $this->prev_tag_open.'<span class="push_button gray" onclick="saveNnext('.$this->cur_page.'), submitFrm(2)">'.$this->prev_link.'</span>'.$this->next_tag_close;
            }
// !!!!!!!! END EDITION !!!!!!!!!!
        ...
        if ($this->next_link !== FALSE && $this->cur_page < $num_pages)
        {
            ...
            $output .= $this->next_tag_open.'<a href="'.$this->base_url.$this->prefix.$i.$this->suffix.'"'.$attributes
                .$this->_attr_rel('next').' class="push_button gray">'.$this->next_link.'</a>'.$this->next_tag_close;
        } 
// !!!!!!!! START EDITION !!!!!!!!!!
        elseif ($this->display_next_link && $this->cur_page >= $num_pages) {
            $output .= $this->next_tag_open.'<span class="push_button gray">'.$this->next_link.'</span>'.$this->next_tag_close;
        }
// !!!!!!!! END EDITION !!!!!!!!!!
		...
		elseif ($this->display_last_link) {
			if(($this->cur_page + $this->num_links) > $num_pages)
			{
                $output .= $this->last_tag_open.'<span>'.$this->last_link.'</span>'.$this->last_tag_close;
			}
			else
			{
				$i = ($this->use_page_numbers) ? $num_pages : ($num_pages * $this->per_page) - $this->per_page;
